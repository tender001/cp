...
public class FilterBQC extends FilterBase{
	...
	public void filter(CodeBean bean, FilterResult result) throws CodeFormatException {
		if(bean.getItemType() == CodeBean.NOITEM){
			...
			pattern = Pattern.compile("(((\\s*\\d+\\s*→((\\s*\\d\\s*-\\s*\\d\\s*)|\\((\\s*\\d\\s*-\\s*\\d\\s*,)*(\\s*\\d\\s*-\\s*\\d\\s*)\\)))\\s*,\\s*)*(\\s*\\d+\\s*→((\\s*\\d\\s*-\\s*\\d\\s*)|\\((\\s*\\d\\s*-\\s*\\d\\s*,)*(\\s*\\d\\s*-\\s*\\d\\s*)\\)))\\s*)");
	...
	/**
	 * 兼容okooo格式
	 * 1→31,2→01,3→31,4→31,5→31,6→01,7→11	1	2	7_1
	 * 9→(11,01),10→(11,01),11→(11,01),13→(31,11),16→(11,01),18→(31,11)	64	128	6_1
	 * @param bean
	 * @param result
	 * @param code
	 * @param codeMaps
...
//		ylist.add("[半全场]10=3-1/0-3/0-1,13=3-1/0-3/0-1,18=3-1/0-3/0-1|3*1");
//		ylist.add("1→1-3,3→3-3,4→3-3,9→1-0	1	2	4_1");
//		ylist.add("6→(3-3,1-3,1-0,0-0),9→3-3,10→(3-3,1-3)	16	32	3_1");
//		ylist.add("45→00,46→33,47→00,48→10");
//		
//		FilterResult fr = new FilterResult();
//		try {
//			FilterBQC bqc = new FilterBQC();
//			CodeBean bean = new CodeBean();
//			bean.setLottype(87);
//			bean.setPlaytype("BQC");
//			bean.setCodeitems("3-3=33,3-1=31,3-0=30,1-3=13,1-1=11,1-0=10,0-3=03,0-1=01,0-0=00");

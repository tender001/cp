...
public class JczqSpfOptimize extends AbstractJczqOptimize {
...
//		Collections.sort(lstResult, new SortByBonus());
	...
	protected String[] createItemToArray(MatchBean bean) {
		...
		for (int i=0;i<ss.length;i++) {
			String spv = getSpValue(bean.spf,"SPF",ss[i]);
//			lst.add(bean.itemid + "=" + ss[i] + "~" + bean.name + "=" + MNAMES.get(ss[i]) + "[" +  spv + "]");
	...
	public static void main(String[] args) {
		JczqSpfOptimize optimize = new JczqSpfOptimize();
		String s = optimize.optimize("SPF|140915001=3/1,140915002=3/1/0", 50, 2, 3,"1","","");

...
import com.alipay.util.BankUtil;
import com.alipay.util.CipherUtil;
...
public class Bank_19pay extends BankBeanImpl {
	...
	*/
	...
	private static String notify_url = RETURN_HOST + "/jxpaynotify.php";
	...
	public static void send(BankBean bean, RbcFrameContext context, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(!bean.getStatus().equals(Status.SUCCESS.CODE)){
			logger.error(bean.getJson().toString());
			return;
		}
		
		String amount = String.valueOf(bean.getMoney()+bean.getMoneyRate());// 总金额，以分为单位

		String version_id = "2.00";
		String order_date = bean.getApplydate();
		String order_id = bean.getOrdernumber();
		String currency = "RMB";
		...
		if(!CheckUtil.isNullString(contents)){
			...
			if (a[2].toUpperCase().equals(merchantSignMsg.toUpperCase())) {
				if(result.equals("F")){
					bean.setErrJson("-100", "收单失败，请联系服务商。");
				}else{
					bean.setErrJson(Status.SUCCESS.CODE, "支付请求成功,请及时查看账户余额。");
				}
			}else{
				bean.setErrJson("-100", "收单失败，请联系服务商。");
			}
			...
			if ("Y".equals(result)) {
				...
				if (rc == 0) {
					if (bean.getStatus().equals("0")) {
						...
						contents = "N";
						logger.error("addMoneySuccess 账户充值确认接口调用失败");
					}
				}

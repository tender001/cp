...
Class.C('lot_name', '11x5');
Class.C('lot_name_ch', '十一选五');
Class.C('lot_name_en', '11x5');
Class.C('lot_id', '54');
Class.C('price', 2);
Class.C('playid', 249);
...
Class
		.C(
				'lot_data_dome',
				{
					244 : '<h5>示例</h5>选号：01<br>开奖：01 02 03 04 05<br>中奖：13元',
					247 : '<h5>示例</h5>选号：01 02<br/>开奖：01 02 03 04 05<br/>中奖：65元',
					249 : '<h5>示例</h5>选号：01 05<br/>开奖：01 02 03 04 05<br/>中奖：6元',
					248 : '<h5>示例</h5>选号：01 02 03<br/>开奖：01 02 03 04 05<br/>中奖：195元',
					250 : '<h5>示例</h5>选号：01 04 05<br/>开奖：01 02 03 04 05<br/>中奖：19元',
					251 : '<h5>示例</h5>选号：01 02 04 05<br/>开奖：01 02 03 04 05<br/>中奖：78元',// r4
					252 : '<h5>示例</h5>选号：01 02 03 04 05<br/>开奖：01 02 03 04 05<br/>中奖：540元',// r5
					253 : '<h5>示例</h5>选号：01 02 03 04 05 06<br/>开奖：01 02 03 04 05<br/>中奖：90元',// r6
					254 : '<h5>示例</h5>选号：01 02 03 04 05 06 07<br/>开奖：01 02 03 04 05<br/>中奖：26元',// r7
					255 : '<h5>示例</h5>选号：01 02 03 04 05 06 07 08<br/>开奖：01 02 03 04 05<br/>中奖：9元',
					245 : '<h5>示例</h5>选号：01 02<br/>开奖：01 02 03 04 05<br/>中奖：130元',// 前二直选
					246 : '<h5>示例</h5>选号：01 02 03<br/>开奖：01 02 03 04 05<br/>中奖：1,170元'// 前三直选
				});
...
Class
		.C(
				'lot_data_wanfa',
				{
					244 : '玩法提示：前一玩法：选<em class="red">1</em>个号码，猜中开奖号码第1个数字即中奖&nbsp;1/11的中奖机会&nbsp;奖金<em class="red">13</em>元。',
					247 : '玩法提示：前二组选玩法：选<em class="red">2</em>个号码与开奖的前2个号码相同即中奖&nbsp;1/55的中奖机会&nbsp;奖金<em class="red">65</em>元。',
					249 : '玩法提示：任选二玩法：选<em class="red">2</em>个号码，猜中开奖号码任意2个数字即中奖&nbsp;1/5.5的中奖机会&nbsp;奖金<em class="red">6</em>元。',
					248 : '玩法提示：前三组选玩法：选<em class="red">3</em>个号码与开奖的前&nbsp;3个号码相同即中奖&nbsp;1/165的中奖机会&nbsp;奖金<em class="red">195</em>元。',
					...
					245 : '玩法提示：前二直选玩法：每行各选<em class="red">1</em>个号码。1/110的中奖机会，奖金<em class="red">130</em>元。',// 前二直选
					246 : '玩法提示：前三直选玩法：每行各选<em class="red">1</em>个号码。1/990的中奖机会，奖金<em class="red">1,170</em>元。'// 前三直选
				});
	...
	if (count > rn) {// 最多命中注数
		if (d > core) {// 胆超过5最大是全中, 任6-8出现
			...
			if (pid === 261 || pid === 264) {
				...
				// 如果任2-4, 从拖中选出余下个数
				max = core < 5 ? Math.c(Math.min(t, 5 - d), core - d) * sp
						: Math.dt(core - d, t - (core - d), rn - d) * sp
			}
		}
	}
	return {
...
Class.extend('checkMaxMoney', function(money, showNode, fn) {// 检测最大金额是否超出
	...
	if (money > limit.max) {
		if (showNode) {// 显示点
			this.getTip().show(
					showNode,
					'<h5>超过最大金额</h5>您好，单个方案最大金额为<strong style="color:red">'
							+ Number(limit.max).rmb(false, 0) + '</strong>元！')
					.setIco(9);
		}
...
Class(
		'SendBuy',
		{
			...
			index : function() {
				var Y = this;
				this.need('#sendbuy').click(
						function() {
							if (Class.C('stop-buy')) {
								return Y.alert('您好，' + Y.C('lot_name_ch')
										+ '暂时停售, 请选择其它彩种投注。');
							}
							Y.chkBuy();
						})
				...
				if (!Y.get('#agreement').prop('checked')) {
					...
					Y.chkLimitCode(list.codes, function() {
						Y.postMsg('msg_login', function() {
							...
							return Y.confirm(tplstr
									.tpl({
										lot : Y.C('lot_name_ch'),
										currentexpect : $(':checkbox:checked')
												.val(),
										expectnum : elist.beishulistsuc
												.split(',').length,
										money : list.totalmoney.rmb(),
										stop : $_sys.zhflag[Y.get('#tzzh')
												.val()],
...
Class(
		'Ball2',
		{
			// 【您选择了<em>0</em>注，共 ￥<em>0.00</em>元 】如中奖，奖金 ￥<em>0.00</em> 元，盈利
			// ￥<em>0.00</em> 元
			showTxt : '【您选择了<em>{$zhushu}</em>注，共 ￥<em>{$totalmoney}</em>元 】如中奖，奖金 {$prix}，盈利  {$yl} ',
			index : function(config) {
				this.putBtn = this.get(config.putBtn).concat(
						this.get(config.put).nodes);
				...
				// });
				this.onMsg('msg_get_choose_code_' + config.msgId, function(
						isKeepCode) {
...
Class(
		...
		{
			// showTxt: '选单金额:[<em
			// class="cm_yellow">{$totalmoney}</em>元]&nbsp;如中奖[奖金：{$prix}，盈利
			// {$yl} ]',
			showTxt : '【您选择了<em>{$zhushu}</em>注，共 ￥<em>{$totalmoney}</em>元 】如中奖，奖金 {$prix}，盈利{$yl} ',
			index : function(config) {
...
Class('Ball>Single', {
	...
	getdtCount : function() {
		...
		return {
			code : [ [
					String.zero('胆: ' + this.ball.dan_data.slice() + ' 拖: '
							+ this.ball.tuo_data.slice()), pid, zs ] ],
			d : d,
			t : t,
			zhushu : zs
		}
	...
	ptchange : function() {
		...
		prix = zhushu > 0 ? this.getPrixRange(this.ball.data.length) : {
			max : 0,
			min : 0
		};
		...
		info = {
			...
			zhushu : zhushu,
			prix : '<em>'
					+ prix.min.rmb(false, 0)
					+ '</em>元'
					+ (prix.max ? (' 至 <em>' + prix.max.rmb(false, 0))
							+ '</em>元' : ''),// 奖金范围
			yl : y1 + (y2 ? (' 至 ' + y2) : ''),
	...
	dtchange : function() {
		...
		y1 = data.zhushu == 0 ? '<em>' + 0 + '</em>元' : '<em >'
				+ (prix.min - tm).rmb(false, 0) + '</em>元';
	...
	getChooseCode : function() {// 传送到列表的数据
		...
		var codeStr = pid == 244 ? [ this.ball.data.join(',') ].concat('-',
				'-', '-', '-', '-').slice(0, 5).join('|') : this.ball.data
				.join(',');
...
Class(
		'Ball2>Multi',
		{
			index : function(ini) {
				...
				this.$(ini.items).each(function(div, i) {
					...
					this.balls[i].onchange = function() {
						...
						// 【您选择了<em>0</em>注，共 ￥<em>0.00</em>元 】如中奖，奖金
						// ￥<em>0.00</em> 元，盈利 ￥<em>0.00</em> 元
							...
							if (count > 0) {
								if (Y.checkMaxMoney(count * Class.C('price'),
									...
									Y.checkBad(count, function(code) {
										...
										Y.clearCode();
										if (!Y.bad.length) {
											Y.postMsg('msg_put_code', code);// 广播号码输出消息,
											// 列表框应该监听此消息
										}
									})
								}
			...
			checkBad : function(zs, fn) {// 检查是否有不能出票的无效号码
				...
				if (this.bad.length) {
					...
					Y.yes.each(function(arr) {
						var code = "";
						code = [ [
								String.zero(arr.concat('-', '-', '-', '-', '-')
										.slice(0, 5).join('|')),
								Class.C('playid'), 1 ] ];
						Y.clearCode();
						Y.postMsg('msg_put_code', code);
					});
					// return this.confirm('您好, 您选择号码中的<br/><span
					// style="'+sty+'">'+String.zero(this.bad.join('<br/>'))+'</span><br/>是无效注，确认剔除并投注吗？',
					// function (){
					// if (zs == 1) {
					// fn(code)
					// }else{
			...
			getChooseCode : function(code) {
				...
				return [ [
						String.zero(code.concat('-', '-', '-', '-', '-').slice(
								0, 5).join('|')), this.getPlayId(),
						this.getCount() ] ];
			}
		});
		...
		'App',
		{
			...
			addTabs : function() {
				...
				playTabs.onchange = function(a, b) {
					...
					var pid = this.get('#play_tabs li').slice(b, b + 1).attr(
							"value");
				...
				zhTabs.onchange = function(a, b) {
					...
					var zid = this.get('#zh_tabs li').slice(b, b + 1).attr(
							"value");
					Class.C('zhid', zid);
					// alert("zid="+zid);
		...
		'CodeList',
		{
			...
			index : function(config) {
				...
				this
						.onMsg(
								'msg_expect_change',
								function(expect, now, endtime) {
									if (this.panel.find('li').size()
											&& !this.C('-is-continue-buy')) {// 如有选号，弹出提示
										var pid = expect + "";
										var ppid = expect - 1;
										var lpid = pid.substr(8);
										if (lpid == '01') {
											var ldate = pid.substr(0, 8);
											var dt = ldate.substr(0, 4) + "-"
													+ ldate.substr(4, 2) + "-"
													+ ldate.substr(6, 2)
													+ " 00:00:00";
											var pdate = Y.getDate(
													Date.parse(Y.getDate(dt))
															- 1000 * 60 * 60
															* 24).format(
													'YYMMDD');
											ppid = (pdate + "")
										...
										this
												.alert('您好，<span style="color:#999">'
														+ ppid
														+ '</span>期已截止，当前期是<span style="color:#FF4E00">'
														+ expect
														+ '</span>期，投注时请确认您选择的期号。');
									}
				...
				this
						.addStyle('.betList li.list-Selected{background-color:#D9F1FF}');
			...
			bindEvent : function(config) {
				...
				this.panel.live('li', 'click', function(e, y) {
					if (!Y.prevSelectedLine || Y.prevSelectedLine != this) {
						Y.get(Y.prevSelectedLine).removeClass('list-Selected')
								.get(this).addClass('list-Selected');
			...
			createLine : function(code) {// 创建一行
				...
				return this.createNode('li', this.panel)
						.html(
								this.lineTpl.format('[' + lot[0] + '] ',
										code[0], z, m));
			},
			...
			change : function(zhushu) {// 变化
				...
				this.zhushu = zhushu;
				this.totalmoney = this.zhushu * this.beishu
						* Class.config('price');
			...
			getData : function() {
				...
				return {
					codes : this.noZero ? arr.join(';') : String.zero(arr
							.join(';')),
					zhushu : this.zhushu,
					beishu : this.beishu,
					totalmoney : this.totalmoney
		...
		'ExpectList',
		{
			...
			index : function() {
				...
				this.onMsg('msg_buy_succeed_zhlist', function() {
					this.get('#expectListBox ul').each(
							function(tr, i) {
								var obs, ochk;
								if (i > 0) {
									obs = tr.getElementsByTagName('li')[2]
											.getElementsByTagName('input')[0];
									ochk = tr.getElementsByTagName('li')[1]
											.getElementsByTagName('input')[0];
									obs.value = 0;
									obs.disabled = true;
									ochk.checked = false;
				...
				// 列表事件
				this.get('#expectListBox').live(
						':text',
						'keyup',
						function(e, Y) {
							Y.flag = false;
							var val = Math.min(2000,
						...
						function() {
							var listdata = Y.postMsg('msg_get_list_data').data;
							var bs = this.parentNode.parentNode.nextSibling
									.getElementsByTagName('INPUT')[0];
							var bss = Y.get('#zh_bs').val() == "" ? 1 : Y.get(
									'#zh_bs').val();
			...
			setAllBs : function(bs) {
				...
				this
						.get('#expectListBox :text')
						.each(
								function(t) {
									if (!t.disabled) {
										t.value = bs;
										t.parentNode.nextSibling
												.getElementsByTagName('em')[0].innerHTML = this
												.getInt(t.value)
												* listdata.totalmoney;
										n++;
									}
				...
				this.get('#sum_m').html(
						(n * bs * 2 * listdata.zhushu).rmb(false, 0));
				this.setTotalData(listdata);
			},
						...
						.each(
								function(t) {
									if (!t.disabled) {
										...
										t.parentNode.nextSibling
												.getElementsByTagName('em')[1].innerHTML = tm;
									}
								}, this);
			},
								...
								function(t) {
									if (n) {// 选中前N期
										...
										if (!t.disabled) {
											...
											t.parentNode.nextSibling
													.getElementsByTagName('em')[0].innerHTML = 0;
										}
									}
								}, this);
			...
			formParam : function(obj) {// 使用参数批量设置 {expect: bs}
				...
				this.get('#expectListBox ul').each(
						function(tr) {
							...
							ochk = tr.getElementsByTagName('li')[1]
									.getElementsByTagName('input')[0];
							em = tr.getElementsByTagName('li')[3]
			...
			setTotalData : function(listdata) {// 更新总期数与金额
				...
				if (cl.length == 1 && code.length > 0) {
					if (code.indexOf('$') > -1) {// 胆拖
						var dt = code.split('$');
						pr = this.getDtPrixRange(dt[0].split(',').length, dt[1]
								.split(',').length);
					} else {
						...
						.each(function(el) {
							if (el.checked) {
								...
								n = parseInt(
										...
										10) || 0;
								bs += n;
								el.parentNode.parentNode.nextSibling.nextSibling
										.getElementsByTagName('em')[1].innerHTML = bs
										* 2 * listdata.zhushu;
								if (isyl && n) {
									var min, max, ylmin, ylmax, bonusMoney;
									if (pr.max) {
										min = (pr.min * n)
												- (2 * listdata.zhushu * bs);
										max = (pr.max * n)
												- (2 * listdata.zhushu * bs);
										ylmin = Math.round((pr.min * n)
												/ (2 * listdata.zhushu * bs)
												* 10000)
												/ 100 + "%";
										ylmax = Math.round((pr.max * n)
												/ (2 * listdata.zhushu * bs)
												* 10000)
												/ 100 + "%";
										bonusMoney = min + "~" + max;
									...
									el.parentNode.parentNode.nextSibling.nextSibling
											.getElementsByTagName('em')[0].innerHTML = n
											* 2 * listdata.zhushu;
									el.parentNode.parentNode.nextSibling.nextSibling.nextSibling
											.getElementsByTagName('em')[0].innerHTML = bonusMoney
											+ "元";
									el.parentNode.parentNode.nextSibling.nextSibling.nextSibling.nextSibling
											.getElementsByTagName('em')[0].innerHTML = ylstr;
								} else {
									el.parentNode.parentNode.nextSibling.nextSibling.nextSibling
								...
								el.parentNode.parentNode.nextSibling.nextSibling
										.getElementsByTagName('em')[0].innerHTML = 0;
								el.parentNode.parentNode.nextSibling.nextSibling
										.getElementsByTagName('em')[1].innerHTML = "--";
								el.parentNode.parentNode.nextSibling.nextSibling.nextSibling
										.getElementsByTagName('em')[0].innerHTML = "--";
								el.parentNode.parentNode.nextSibling.nextSibling.nextSibling.nextSibling
										.getElementsByTagName('em')[0].innerHTML = "--";
						...
						.each(
								function(tr) {
									...
									if (et <= now) {
										...
										var lab = tr.getElementsByTagName('li')[1]
												.getElementsByTagName('label')[0];
										lab.className = 'red';
										lab.appendChild(document
												.createTextNode('[当前期]'));
										...
										bsinput.value = this.get('#zh_bs')
												.val();
										tr.getElementsByTagName('li')[3]
												.getElementsByTagName('em')[0].innerHTML = (this
												.getInt(bsinput.value) * listdata.totalmoney)
												.rmb(false, 0);
									}
									...
									tr.getElementsByTagName('li')[0]
											.getElementsByTagName('span')[0].innerHTML = n++;
								}, this);
			...
			getParam : function() {
				if (this.expectlistsuc.split(",").length > 1) {
					...
					return {
						...
						'ZjCut' : this.get('#tzzh').val(),// .prop('checked')
						// ? 1 : 0,
		...
		'BuyProject',
		{
			...
			addMsg : function() {
				this.onMsg('msg_load_expect_list', function(list1) {
					...
					list1.each(function(item) {
						opts.add(new Option(item.pid
								+ (item.iscurrent ? '[当前期]' : ''), item.pid, 0,
								0))
					});
			...
			getHTML : function() {
				...
				if (this.expect.value == '')
					return this.alert('您好，请输入投入期数！');
				if (this.mult.value == '')
					return this.alert('您好，请输入起始倍数！');
				if (this.fullIncomeBtn.checked) {
					if (this.fullIncome.value == '')
						return alert('您好，请输入全程收益率！');
				}
				;
				...
				var base1 = false, base2 = false, base3 = false;
				var pre = l;
				base1 = bonusMoney / (1 + this.fullIncome.value / 100)
						- buyMoney;
				base2 = 1.1;
				if (this.beforeIncomeBtn.checked) {
					if (this.beforeExpect.value == '')
						return this.alert("您好，请输入前几期追号期数！");
					if (this.beforeIncome.value == '')
						return this.alert('您好，请输入前几期的预定收益率！');
					if (this.backIncome.value == '')
						return this.alert('您好，请输入前几期之后收益率！');
					pre = this.beforeExpect.value;
					base1 = bonusMoney / (1 + this.beforeIncome.value / 100)
							- buyMoney;
					base2 = bonusMoney / (1 + this.backIncome.value / 100)
							- buyMoney;
				}
				;
				if (base1 < 1 || base2 < 1)
					return this.alert('您好，您的当前方案不适合倍投！');
				if (this.sumIncomeBtn.checked) {
					if (this.sumIncome.value == "")
						return this.alert("您好，请输入累计收益的最小金额！");
				...
				if (this.sumIncomeBtn.checked) {
					...
					for ( var i = 1; i < l; i++) {
						...
						bs = Math.max(bs, base1 ? Math
								.ceil((sum / (i < pre ? base1 : base2))
										.toFixed(4)) : 0, base3 ? Math
								.ceil((sum + minMoney) / base3) : 0);
						if (bs > 1000000) {
				...
				if (this.isArray(data)) {
					...
					for ( var i = 0; i < data.length; ++i) {
						...
						if (el.value == data[i].expect) {
							el.checked = true;
							el.parentNode.parentNode.nextSibling
									.getElementsByTagName('INPUT')[0].value = data[i].mult;
						}
					}
					var listdata = this.postMsg('msg_get_list_data').data;
...
Class(
		'openCodeList',
		{
			omissionurl : "/cpdata/omi/" + Class.C('lot_id')
					+ "/yilou/miss.xml?rnd=" + Math.random(),
			// winlisturl:"/trade/ranking.go?find=" + Class.C('lot_id') +
			// "&name=today&rnd=" + Math.random(),
			// castlisturl:"/user/querycastlist.go?gid=" + Class.C('lot_id') +
			// "&rnd=" + Math.random(),
			index : function() {
			...
			showlist : function(n) {// 显示开奖号码列表 球
				...
				if ((npid + "").substr(8, 2) == "01") {
					net = (Y.getDate(Date.parse(Y.getDate(lpid.t)) + 1000 * 60
							* 60 * 24).format('YY-MM-DD'))
							+ " 09:10:00";
				} else {
					net = (Y
							.getDate(Date.parse(Y.getDate(lpid.t)) + 1000 * 60 * 10)
							.format('YY-MM-DD hh:mm:ss'));// 2013-02-26
					// 10:20:00
				}

				this.get('#lastopenexpect').html(
						'<em>{1}</em> 期开奖号码'.format(llpid.p));
				...
				this.get('#lastopencode').html(
						ctpl
								.format(llcc[0], llcc[1], llcc[2], llcc[3],
										llcc[4]));
				// alert("npid="+npid+" \r\n
				...
				if (npid != this.C('currentExpect')) {
					st = 1; // 开奖倒计时
					this.get('#lastopenexpect').html(
							'<em>{1}</em> 期开奖号码'.format(lpid.p));
					var ctpl = ' <b class="es1 es">{1}</b><b class="es2 es">{2}</b><b class="es3 es">{3}</b><b class="es4 es">{4}</b><b class="es5 es">{5}</b>';
					var llcc = lpid.c.split(",");
					this.get('#lastopencode').html(
							ctpl.format(llcc[0], llcc[1], llcc[2], llcc[3],
									llcc[4]));
				}
				...
				// this.get('#kaijianginfo').html('<span>前一：</span><s
				// id="qian1">{1}</s><span>前二：</span><s
				// id="qian2">{2}</s><span>前三：</span><s
				// id="qian3">{3}</s>'.format(cc[0],cc[0]+'&nbsp;&nbsp;'+cc[1],cc[0]+'&nbsp;&nbsp;'+cc[1]+'&nbsp;&nbsp;'+cc[2]));

				var ld = this.listdata.slice(dl - (9 - 0), dl);// 列表里面的所以值
				// 都在ld里面
				for ( var i = 0; i < ld.length; i++) {
					var ul = "";
					if (pid == 245) {// 前二
						ul = this.get('.opencodebox', "#opencodelist_2").slice(
								i);
					} else if (pid == 246) {// 前三
						ul = this.get('.opencodebox', "#opencodelist_3").slice(
								i);
					} else {
						ul = this.get('.opencodebox', "#opencodelist").slice(i);
					}
					ul.attr("expect", ld[i].p).attr("endtime", ld[i].t).attr(
							"ol", 0).attr("codes", "");
					// ul.removeClass("cm_11ydj_text_hover").addClass("cm_11ydj_text_text");
					// var castinfolist = this.get('.cm_11ydj_text_xlhover div',
					// ul);
					// castinfolist.html("");
					var opencode = ld[i].c;
					var opentime = ld[i].t;
					...
					if (opencode != "") {
						if (pid == 245) {// 前二
							...
							this.get('.pid', ul).html(
									"" + ld[i].p.substr(2) + "");
							this.showomiss(omissdata, opencodelen, opencode,
									ul, opentime);
						} else if (pid == 246) {// 前三
			...
			showomiss : function(data, opencodelen, opencode, ul, opentime) {// 填遗漏
				// (遗漏
				// ，
				// 开奖号码
				// 长度，开奖号码，指定那一行)
				...
				this
						.ajax({
							...
							cache : false,// 是否缓存
								...
								var et = (Y
										.getDate(Date.parse(Y.getDate(ed)) + 1000 * 95)
										.format('YY-MM-DD hh:mm:ss'));
								this.clock.end('loading...');
								var diff = this.getDate(et)
										- this.getDate(this.edata);
								...
								if (diff > 0) {
									...
									var __oncd = {
										...
										change : function(times, now) {
											var lt = times[1] * 60 * 60
													+ times[2] * 60 + times[3];
											var wlt = lt > 314 ? 314 : lt;

											this
													.get('#kaijiangopencode')
													.html(
															'{3}:{4}'.format
																	.apply(
																			'{3}:{4}后开奖<span><em style="width:'
																					+ wlt
																					+ 'px"></em></span>',
																			times)
																	.replace(
											...
											if (lt == 0 || lt == 1) {
												...
												this.get('#kaijiangopencode')
														.html('正在开奖……');
												clearInterval(window.refreshKjTime);
												window.refreshKjTime = setInterval(
														function() {
															Y
																	.postMsg('opencodelist_getlistdata');
														}, 1000 * 10);
											}
										}
									};
									...
									this.get('#kaijiangopencode')
											.html('正在开奖……');
									// timebar.html('<em>'+pid+'</em> 期'+ '<b
									// >正在开奖……</b>');
									clearInterval(window.refreshKjTime);
									window.refreshKjTime = setInterval(
											function() {
												Y
														.postMsg('update_opencodelist');
											}, 1000 * 10);
								}
							}
						});
			...
			getnextpid : function(pid) {
				...
				if (lpid == Class.C('sump')) {// 判断是不是最后一期
					var dt = ldate.substr(0, 4) + "-" + ldate.substr(4, 2)
							+ "-" + ldate.substr(6, 2) + " 00:00:00";
					var ndate = Y.getDate(
							Date.parse(Y.getDate(dt)) + 1000 * 60 * 60 * 24)
							.format('YYMMDD');
					npid = ndate + "01";
				} else {
					npid = pid * 1 + 1;
							...
							retry : 1,
								...
								this
										.qXml(
												...
												function(o, i) {
													...
													var buynum = this
															.get(
																	'.cm_11ydj_text_xlhover .buynum',
																	ul);
													var oknum = this
															.get(
																	'.cm_11ydj_text_xlhover .oknum',
																	ul);
													var buymoney = this
															.get(
																	'.cm_11ydj_text_xlhover .cm_red',
																	ul);
													buynum
															.html(buynum.html() * 1 + 1);
													if (st == 1) {
														oknum
																.html(oknum
																		.html() * 1 + 1);
													...
													var castinfolist = this
															.get(
																	'.cm_11ydj_text_xlhover div',
																	ul);
													var purl;
													if (type == 1) {
														purl = $_sys
														...
														purl = "/useraccount/usertouzhu/xchase.html?tid="
																+ projid
																+ "&lotid="
																+ Class
																		.C('lot_id');
													...
													this.rm = bonus * 1;
													if (bonus > 0) {
														// castinfolist.html(castinfolist.html()+'<a
														// href="' + purl + '"
														// target="_blank"><p>'
														// + bonus + '</p><span
														// title="方案金额：' +
														...
														infohtml = '<a href="'
																...
																+ '" target="_blank" title="'
																+ ttitle
																+ '"><p><em class="cm_11ydj_zjico '
																+ icss
																+ '"><b class="cm_11ydj_zjnum">'
																+ this.rm
																		.rmb(
																				true,
																				0)
								...
								if (arr1.length < 1) {
									if (arr2.length == 1) {
										$("#curState")
												.html(
														"<a class=\"cm_11ydj_qb cm_left\" href=\"/useraccount/usertouzhu/xchase.html?tid="
																+ arr2[0]
																+ "&lotid="
																+ Class
																		.C('lot_id')
																+ "\" target=\"_blank\" title=\"成功待开奖方案\"></a>");
									} else if (arr2.length > 1) {
										$("#curState")
												.html(
														"<a class=\"cm_11ydj_qb cm_left\" href=\"/useraccount/usertouzhu/chase.html\" target=\"_blank\" title=\"成功待开奖方案\"></a>");
									}
								} else if (arr1.length == 1) {
									if (arr2.length < 1) {
										$("#curState")
												.html(
														"<a class=\"cm_11ydj_qb cm_left\" href=\"/11x5/viewpath.html?lotid=54&projid="
																+ arr1[0]
																+ "\" target=\"_blank\" title=\"成功待开奖方案\"></a>");
									} else {
										$("#curState")
												.html(
														"<a class=\"cm_11ydj_qb cm_left\" href=\"/useraccount/usertouzhu/userbetlist.html\" target=\"_blank\" title=\"成功待开奖方案\"></a>");
									}
								} else if (arr1.length > 1) {
									$("#curState")
											.html(
			...
			showcastinfo : function() {
				...
				if (pid == 245) {
					this
							.get(
									'.cm_11ydj_st,.cm_11ydj_jz,.cm_11ydj_st_gray,.cm_11ydj_qb,.cm_11ydj_jz_gray,.cm_11ydj_qb_gray',
									"#opencodelist_2")
							.each(
									function(casttag, i) {
										...
										this
												...
												.click(
														function() {
															var castinfo = Y
																	.get(casttag.parentNode.parentNode.parentNode.nextSibling);
															var ul = Y
																	.get(casttag.parentNode.parentNode.parentNode.parentNode);
															if (castinfo
				...
				var opencode_n = "";
				if (id == 245) {
					opencode_n = this.get('.opencodebox[ol=1]',
							"#opencodelist_2");
				} else if (id == 246) {
					opencode_n = this.get('.opencodebox[ol=1]',
							"#opencodelist_3");
					...
					opencode_n = this
							.get('.opencodebox[ol=1]', "#opencodelist");
				}
			...
			createDayOpenCodeHtml : function() {
				var htmlul = "";
				for ( var i = 1; i < 14; i++) {
					htmlul += "<tr><th><span id=span" + String.zero(i) + ">"
							+ String.zero(i)
							+ "</span></th><td><span id=dopencode"
							+ String.zero(i) + "></span></td>"
							+ "<th><span id=span" + (13 + i) + ">" + (13 + i)
							+ "</span></th><td><span id=dopencode" + (13 + i)
							+ "></span></td>" + "<th><span id=span" + (26 + i)
							+ ">" + (26 + i)
							+ "</span></th><td><span id=dopencode" + (26 + i)
							+ "></span></td>" + "<th><span id=span" + (39 + i)
							+ ">" + (39 + i)
							+ "</span></th><td><span id=dopencode" + (39 + i)
							+ "></span></td>" + "<th><span id=span" + (52 + i)
							+ ">" + (52 + i)
							+ "</span></th><td><span id=dopencode" + (52 + i)
							+ "></span></td>" + "<th><span id=span" + (65 + i)
							+ ">" + (65 + i)
							+ "</span></th><td><span id=dopencode" + (65 + i)
							+ "></span></td>";
				}
			...
			updateDayOpenCode : function(d) {
				...
				this.ajax({
					url : "/cpdata/game/" + Class.C('lot_id') + "/day/" + riqi2
							+ ".json",
					type : "get",
					cache : false,
					dataType : "json",

...
Class( 'LineSelector', {
	...
	bindEvent : function() {
		...
		this.get(this.vsLine).hover( function() {
			...
			this.children[10].style.backgroundColor = '#fee6ad';
			this.children[11].style.backgroundColor = '#fee6ad';
			...
			this.children[10].style.backgroundColor = '';
			this.children[11].style.backgroundColor = '';
	...
	createTrHtml : function(oTr) {
		...
		if (play_name == 'rqspf') {
			...
			tr_html = '<tr>' + 
					  ...
					  '</tr>' +
					  '<tr>' +
						  '<td colspan="3" style="padding-left:40px;">' + td_html + '</td>' +
					  '</tr>';
		}
		return tr_html;
	}
	...
	sxds:function(data){
		 ...
		 var tableTpl=['<colgroup>'+
          ...
          '</colgroup>',//0
	       ...
	       '<a title="{$hn}"  href="" target="_blank" id="hn{$mid}"><em id="gtid{$mid}" class="ew_e">{$short_hn}</em></a>'+	      
	       '</td>'+
		...
		r.each(function(row,i){
			...
			row.sxp0=$_sys.getsp(row.sxp,"sxp3")?parseFloat($_sys.getsp(row.sxp,"sxp3")).rmb(false,2):'--';
			row.sxp1=$_sys.getsp(row.sxp,"sxp2")?parseFloat($_sys.getsp(row.sxp,"sxp2")).rmb(false,2):'--';
			row.sxp2=$_sys.getsp(row.sxp,"sxp1")?parseFloat($_sys.getsp(row.sxp,"sxp1")).rmb(false,2):'--';
			row.sxp3=$_sys.getsp(row.sxp,"sxp0")?parseFloat($_sys.getsp(row.sxp,"sxp0")).rmb(false,2):'--';
				
			
			row.rz=$_sys.getrz(row.rs,3);//this.getsp(row.sxp,"rz");
			row.rs=parseFloat($_sys.getrs(row.rs,3)).rmb(false,2);//parseFloat(this.getsp(row.sxp,"rs")).rmb(false,2);
			...
			if (out_of_date&&row.rz!=''){//已经开出赛果的
				row.shuju='<span class="red eng">'+row.bf+'</span>';
				row.sxp0str=(row.rz==3?('<span class="red">'+row.rs+'</span>'):'--');
				row.sxp1str=(row.rz==2?('<span class="red">'+row.rs+'</span>'):'--');
				row.sxp2str=(row.rz==1?('<span class="red">'+row.rs+'</span>'):'--');
				row.sxp3str=(row.rz==0?('<span class="red">'+row.rs+'</span>'):'--');				

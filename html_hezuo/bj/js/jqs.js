	...
	createTrHtml : function(oTr) {
		...
		if (play_name == 'rqspf') {
			...
			tr_html = '<tr>' + 
					  ...
					  '</tr>' +
					  '<tr>' +
						  '<td colspan="3" style="padding-left:12px;">' + td_html + '</td>' +
					  '</tr>';
		}
		return tr_html;
	}
...
Class('LoadExpect',{
	...
	jqs:function(data){
		...
		r.each(function(row,i){
			...
			row.short_gn=row.gn.substr(0,6);
			row.short_hn=row.hn.substr(0,6);
			row.index=row.mid;
			...
			row.jqs0=$_sys.getsp(row.jqs,"jqs0")?parseFloat($_sys.getsp(row.jqs,"jqs0")).rmb(false,2):'--';
			row.jqs1=$_sys.getsp(row.jqs,"jqs1")?parseFloat($_sys.getsp(row.jqs,"jqs1")).rmb(false,2):'--';
			row.jqs2=$_sys.getsp(row.jqs,"jqs2")?parseFloat($_sys.getsp(row.jqs,"jqs2")).rmb(false,2):'--';
			row.jqs3=$_sys.getsp(row.jqs,"jqs3")?parseFloat($_sys.getsp(row.jqs,"jqs3")).rmb(false,2):'--';
			row.jqs4=$_sys.getsp(row.jqs,"jqs4")?parseFloat($_sys.getsp(row.jqs,"jqs4")).rmb(false,2):'--';
			row.jqs5=$_sys.getsp(row.jqs,"jqs5")?parseFloat($_sys.getsp(row.jqs,"jqs5")).rmb(false,2):'--';
			row.jqs6=$_sys.getsp(row.jqs,"jqs6")?parseFloat($_sys.getsp(row.jqs,"jqs6")).rmb(false,2):'--';
			row.jqs7=$_sys.getsp(row.jqs,"jqs7")?parseFloat($_sys.getsp(row.jqs,"jqs7")).rmb(false,2):'--';			
			
			row.rz=$_sys.getrz(row.rs,4);//this.getsp(row.jqs,"rz");
			row.rs=parseFloat($_sys.getrs(row.rs,4)).rmb(false,2);//parseFloat(this.getsp(row.jqs,"rs")).rmb(false,2);
			...
			if (out_of_date&&row.rz!=''&&(parseFloat(row.rs)>0)){//已经开出赛果的//&&row.bf!=''				
				row.jqs0str=(row.rz==0?('<span class="red">'+row.rs+'</span>'):'--');
				row.jqs1str=(row.rz==1?('<span class="red">'+row.rs+'</span>'):'--');
				row.jqs2str=(row.rz==2?('<span class="red">'+row.rs+'</span>'):'--');
				row.jqs3str=(row.rz==3?('<span class="red">'+row.rs+'</span>'):'--');
				row.jqs4str=(row.rz==4?('<span class="red">'+row.rs+'</span>'):'--');
				row.jqs5str=(row.rz==5?('<span class="red">'+row.rs+'</span>'):'--');
				row.jqs6str=(row.rz==6?('<span class="red">'+row.rs+'</span>'):'--');

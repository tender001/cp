    ...
    Class('Choose', {
        ...
        addLive: function (){
            this.tbody.live('span.'+this.focusCss, 'mousedown', function (e, Y){//点击选中项
                var me = Y.get(this),
                    tr = me.parent('tr'),
					tr2 = Y.twoLine ? tr.data('linkTR') : tr;
                me.hide();//隐藏点击过的span
    ...
    Class('LoadExpect',{
    	...
    	sfc:function(data){
   		 ...
   		 var tableTpl=['<TABLE class=dc_table border=0 cellSpacing=0 cellPadding=0 width="100%">'+
              ...
              '<TABLE id=d_{$enddate} class=dc_table border=0 cellSpacing=0 cellPadding=0 width="100%">'+
              '<COLGROUP><COL width="10%"><COL width="8%"><COL width="10%"><COL width="12%"><COL width="9%">'+
              '<COL width="7%"><COL width="7%"><COL width="7%"><COL width="8%"><COL width="8%"><COL width="8%"><COL width="5%"></COLGROUP>',//2对阵table控制
              '<TBODY>'+
              '<TR style="DISPLAY: none" lg="{$mname}" isend="1" odds="{$nsfc}" pdate="{$itemid}" pendtime="{$et}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}"  homeTeam="*{$shn}" guestTeam="*{$sgn}">'+
              ...
              '</TBODY>',//3隐藏对阵
              '<TBODY>'+
              '<TR lg="{$mname}" isend="0" odds="{$nsfc}" pdate="{$itemid}" pendtime="{$et}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}"  homeTeam="{$hn}" guestTeam="{$gn}">'+
              '<TD rowSpan=2><LABEL for=m{$itemid}><INPUT id=m{$itemid} class=i-cr value={$itemid} CHECKED type=checkbox name=m{$itemid}>{$name}</LABEL></TD>'+
	             '<TD style="BACKGROUND: {$cl}; COLOR: #fff" class=league rowSpan=2><A title={$lmname} href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</A></TD>'+
	             '<TD rowSpan=2><SPAN class="eng end_time" title="开赛时间：{$mt}">{$short_et}</SPAN><SPAN style="display:none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</SPAN> </TD>'+
	             '<TD><A title={$gn} href="" target="_blank" id="gn{$itemid}">{$gn}</A></TD>'+
...
//引导启动类
    Class({
    		...
    		this.ajax({
    				url:"/cpdata/omi/jclq/odds/odds.xml",
                         ...
                         this.qXml('//row', data.xml, function (u, i){
                        	 Y.get("#mn"+u.items.xid).attr("href","http://info.gucheng.159cai.com/index.php?controller=lqmain&lid="+u.items.lid+"&sid="+u.items.sid);
                        	 Y.get("#hn"+u.items.xid).attr("href","http://info.gucheng.159cai.com/index.php?controller=lqteam&action=index&sid=1&tid="+u.items.htid);
                        	 Y.get("#gn"+u.items.xid).attr("href","http://info.gucheng.159cai.com/index.php?controller=lqteam&action=index&sid=1&tid="+u.items.gtid);
                        	 Y.get("#ox"+u.items.xid).attr("href","http://info.gucheng.159cai.com/index.php?controller=fenxi&mid="+u.items.mid);
                        	 Y.get("#oz"+u.items.xid).attr("href","http://info.gucheng.159cai.com/index.php?controller=lqeodds&mid="+u.items.mid);
        ...
        _index:function (){
            ...
            this.lib.Buy();//购买类
            this.lib.Clock('#sysTimeDisplay');//时间

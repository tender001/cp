...
//                var tdCSS=Y.get(this).find('.chbox').attr("checked")==true?"h_br label_cd":"h_br h_brx";
//                Y.get(this).attr("class",tdCSS);
            	Y.get(this).find(".label_cd").removeClass("h_brx");
            });
    ...
    Class('LoadExpect',{
    	...
    	sf:function(data){
    		 ...
    		 var tableTpl=['<TABLE class=dc_table border=0 cellSpacing=0 cellPadding=0 width="100%">'+
		           ...
		           '<td style="background:{$cl}; color: #fff" class=league><a title="{$lmname}" href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</a></td>'+
		           '<td><span class="eng end_time" title="开赛时间：{$mt}">{$short_et}</span><span style="display:none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</span></td>'+
		           '<td style="text-align: left; padding-left: 18px; cursor: pointer" class=h_br><div class="dz_dv" title={$gn}><input class=chbox value=0 type=checkbox style="display: none"><span class="eng b span_right"><b class="b_right">{$sp0}</b> <em id="gtid_{$itemid}" class="em_right">{$gn}</em></span><s class="s_right"></s></div></td>'+
		           '<td style="text-align: right; padding-right: 18px; cursor: pointer" class=h_br><div class="dz_dv" title={$hn}><input class=chbox value=3 type=checkbox style="display: none"><s class="s_left">&nbsp;</s><span class="eng b span_left"><em id="htid_{$itemid}" class="em_left">{$hn}</em> <b class="b_left">{$sp3}</b></span></div></td>'+
		           '<td>'+
...
Class('SPUpdate', {
    ...
    index:function (isgg){
        var Y = this;
        this.xml = '/static/public/jclq/xml/live/'+this.C('spXml')+'_'+(isgg ? 2 : 1)+'.xml';
        setInterval(function() {
            Y.querySP()
        },this.interval);
    },

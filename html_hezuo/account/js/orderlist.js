	 ...
	 index:function (config){
	    ...
	    showinfo:function(){
			...
			if(lotid!=""&&typeof(lotid) != 'undefined'){
				...
				P.getlist($("#begintime").val(), $("#endtime").val(), lotid,$("#seltype").val(),1,10,0,0);
        	}else{
        		P.getlist($("#begintime").val(), $("#endtime").val(), "0",$("#seltype").val(),1,10,0,0);
        	}
...
//							if(gid==4){gid="04";}
						...
						pagehtml += '<a class="a2" style="margin-right:5px" title="上一页 " onclick=\"' + fn + '('+(pn-1>0?(pn-1):1)+','+ps+','+tp+','+tr+');\" href="javascript:void(0)">上一页</A>';
						var min=0;
						var max=0;
						if ( tp > maxshow){
						...
						pagehtml+='<span class="ac">第<INPUT onkeydown="if(event.keyCode==13){page(Y.getInt(this.value));return false;}" id=govalue class="ac" onkeyup="this.value=this.value.replace(/[^\\d]/g,\'\');if(this.value>'+tp+')this.value='+tp+';if(this.value<=0)this.value=1"  name=page>页</span><a class="go" href="#" onclick=\"' + fn + '(Y.getInt($(\'#govalue\').val()),'+ps+','+tp+','+tr+');\"></a><a class="a2" style="margin-left:10px" onclick=\"' + fn + '('+(pn+1>tp?tp:(pn+1))+','+ps+','+tp+','+tr+');\"  href="javascript:void(0)">下一页</a><a class="a1" style="margin-left:5px" onclick=\"' + fn + '('+tp+','+ps+','+tp+','+tr+');\" href="javascript:void(0)">尾页</a><span class="gy">共'+tp+'页，'+tr+'条记录</span>';

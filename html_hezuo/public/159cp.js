...
getpage=function(pn,ps,tp,tr,fn){
	var pagehtml = "";
	var maxshow=10; 
	pagehtml+="<a class=\"a1\" href=\"javascript:void(0);\" style=\"margin-right:5px\" onclick=\"" + fn + "(1,"+ps+","+tp+","+tr+");\">首页</a>";
	if(pn>1){
		pagehtml+="<a class=\"a2\" href=\"javascript:void(0);\" style=\"margin-right:5px\" onclick=\"" + fn + "("+((pn*1)-1)+","+ps+","+tp+","+tr+");\">上一页</a>";
	}else{
		pagehtml+="<a class=\"a2\" href=\"javascript:void(0);\" style=\"margin-right:5px\">上一页</a>";
	}
	 var min=0;
     var max=0; 
     if (tp > maxshow){
    ...
    pagehtml+="<span class=\"ac\">第<input class=\"ac\" name=\"n_page\" id=\"n_page\" onkeydown=\"if(event.keyCode==13){"+fn+"(Y.getInt(this.value),"+ps+","+tp+","+tr+");return false;}\" onkeyup=\"this.value=this.value.replace(/[^\\d]/g,\'\');if(this.value>"+tp+")this.value="+tp+";if(this.value<=0)this.value=1\"/>页</span>";
 	pagehtml+="<a class=\"go\" id=\"go_page\"  onclick=\""+fn+"(Y.getInt($(\'#n_page\').val()),"+ps+","+tp+","+tr+");\"></a>";
 	pagehtml+="<span class=\"gy\">共"+tp+"页，"+tr+"条记录</span>";
 	pagehtml+="</div>";
 	
	if(pn<tp){
		pagehtml+="<a class=\"a2\" href=\"javascript:void(0);\" style=\"margin-right:5px\"  onclick=\"" + fn + "("+((pn*1)+1)+","+ps+","+tp+","+tr+");\">下一页</a>";
	}else{
		pagehtml+="<a class=\"a2\" href=\"javascript:void(0);\" style=\"margin-right:5px\">下一页</a>";
	}
	
	pagehtml+="<a class=\"a1\" href=\"javascript:void(0);\" style=\"margin-left:5px\"  onclick=\"" + fn + "("+tp+","+ps+","+tp+","+tr+");\">尾页</a>";
	pagehtml+="<div class=\"mtxs\">";
	pagehtml+="每页显示";
	pagehtml+="<label class=\"label_l\" for=\"pg10\">";
	pagehtml+="<input id=\"pg10\" type=\"radio\" name=\"pages\" onclick=\"" + fn + "(1,10,0,"+tr+");\"";
	if (ps==10){
		pagehtml+=" checked=\"checked\"";
	}		
	pagehtml+=" value=\"10\">10条</label> ";

	pagehtml+="<label for=\"pg50\">";
	pagehtml+="<input id=\"pg50\" type=\"radio\" name=\"pages\" onclick=\"" + fn + "(1,50,0,"+tr+");\"";
	if (ps==50){
		pagehtml+=" checked=\"checked\"";
	}	
	pagehtml+=" value=\"50\">50条</label>";
	pagehtml+="</div>";
    return pagehtml;
};
/**
...
$_sys.showzhanjii = function (lotid,uid,au,ag,func){
	...
	return uid=='******'?$_sys.showzhanji(au,ag):('<a href="javascript:void(0);" onclick="Y.openUrl(\'/game/zhanji.html?lotid='+lotid+'&uid='+uid+'&func='+func+'\',806,600)">'+$_sys.showzhanji(au,ag)+'</a>');
};
...
$_sys.showzhanjiname = function (lotid,uid,func){
	...
	return '<a href="javascript:void(0);" onclick="Y.openUrl(\'/game/zhanji.html?lotid='+lotid+'&uid='+uid+'&func='+func+'\',806,600)"><b>'+uid+'</b></a>';
};

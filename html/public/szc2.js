    ...
    Class('LoadExpect',{
		...
		_showkj : function(row) {    		
			...
			$("#showdetails").hover(function(){
				$("#kj_details").toggle();
				$("#showdetails").toggleClass('cur');
			});
			$("#showprompt").click(function(){
				$("#promptdetails").toggle();
				$("#showprompt").toggleClass('cur');
				if($("#showprompt").html()=="收起"){
					$("#showprompt").html("展开");
				}else{
					$("#showprompt").html("收起");
				}
			...
			$("#myproject").click(function(){
				Y.postMsg('msg_login', function (){
					location.href ='/account/orderlist.html?lotid='+row.gid+'';
...
//号码列表
    Class('CodeList', {
        ...
        //选中行
            ...
            this.panel.live('div', 'click', function (e, y){
        ...
        addCode: function (code){
            ...
            if (code.length) {
                ...
                for (var i = 0, j = code.length; i < j; i++) {
                    ...
                    this.need('.a2', li).click(function (e, Y){
                ...
                this.zsInput.val(Y.get("#beishu").val())
                this.beishu = Y.get("#beishu").val();
                this.change(this.getCount())
            }
        },
        createLine: function (code){//创建一行
        	if((code[0]+code[1]).length>33){
        		return this.createNode('div', this.panel).addClass("r_a_p").addClass("r_a_pp").html(this.lineTpl.format(String.zero(code[0].join(',')), this.splitChar, String.zero(code[1].join(this.rightSplit)),parseInt(code[2]),String.zero(code[2])*2));
			}else{
				return this.createNode('div', this.panel).addClass("r_a_p").html(this.lineTpl.format(String.zero(code[0].join(',')), this.splitChar, String.zero(code[1].join(this.rightSplit)),parseInt(code[2]),String.zero(code[2])*2));
			}
        	
        },
        removeLine: function (p){//删除一行
            if (p == this.prevSelectedLine) {
                this.prevSelectedLine = null
            }
            this.removeNode(p);
            this.change(this.getCount())
        ...
        getCount: function (){//计算总注数
            var Y = this;
            return this.get('div', this.panel).nodes.reduce(function (a, b){
        ...
        getData: function (){//03,07,10,18,21,22|08$10,13,15,18,20,25,30|05,08,15
            var arr = [];
            this.get('div', this.panel).each(function (a){
    ...
    Class('DsUpload', {
        ...
        addElements: function (config){
            ...
            this.priceChange = this.bsInput.onchange = this.zsInput.onchange = function (){
                ...
                Y.get("#sczsinput").html(data.zhushu);
               ...
               // this.input.input.doProp('select')
	            ...
	            this.need(config.addbs).click(function (){
	            	if(Y.getInt(Y.zsInput.val())<1){
	            		Y.getTip().show(Y.input,'请输入您要上传的注数').setIco(3);
	            		return
	            	}
	            	var limit, data = {
	                    zhushu: Y.getInt(Y.zsInput.val()),
	                    beishu: Y.getInt((parseInt(Y.bsInput.val())+1)>99?99:(parseInt(Y.bsInput.val())+1)),
	                    totalmoney:Y.getInt(Y.zsInput.val()*Y.getInt((parseInt(Y.bsInput.val())+1)>99?99:(parseInt(Y.bsInput.val())+1))*Class.config('price'))
	                }
	                Y.bsInput.val(data.beishu);
	                Y.moneySpan.html((data.totalmoney).rmb());
	                Y.postMsg('msg_list_change', data);
	                limit = Y.getLimit();
	                if (data.totalmoney < limit.min) {
	                    Y.getTip().show(Y.input,'<h5>投注金额限制</h5>您发起的方案金额不能少于'+limit.min.rmb()+'元, 请修改!').setIco(3);
	               // this.input.input.doProp('select')
	                }else if(data.totalmoney > limit.max){
	                    Y.getTip().show(Y.input,'<h5>投注金额限制</h5>您发起的方案金额不能大于'+limit.max.rmb()+'元, 请修改!').setIco(3);
	                //this.input.input.doProp('select')
	                }else{
	                    Y.getTip().hide()
	                }
	            ...
	            this.need(config.lessbs).click(function (){
	            	...
	            	var limit, data = {
	                    zhushu: Y.getInt(Y.zsInput.val()),
	                    beishu: Y.getInt((Y.bsInput.val()-1)>1?(Y.bsInput.val()-1):1),
	                    totalmoney:Y.getInt(Y.zsInput.val()*Y.getInt((Y.bsInput.val()-1)>1?(Y.bsInput.val()-1):1)*Class.config('price'))
	                }
...
//排列直选选择器
    Class('Choose_base>PLChoose',{
        showTxt: '您选择了<b> {$zhushu} </b>注，共<b> {$totalmoney} 元</b> ',
...
//排列号码框
    Class('CodeList>PLCodeList', {
        noZero: true,
        lineTpl: '<span class="span1">标准选号</span><span class="span2"> {1}{2}</em> </span>'+
    	'<span class="span3"> <s>{3}注</s>  <a href="javascript:void(0);" class="a1" title="修改"></a><a href="javascript:void(0);" class="a2" title="删除"></a></span>',
        createLine: function (code){//创建一行
            var tc, type, pn;
            tc = [];
            ...
            return this.createNode('div', this.panel).addClass("r_a_p").html(this.lineTpl.format(tc.join('|'), type,code[3]));
        },

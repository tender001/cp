...
$_sys.showzhanji= function(au,ag){
	...
	var html='';
	 ...
	 if(Y.getInt(crownNum)+Y.getInt(sunNum)+Y.getInt(moonNum)+Y.getInt(starNum)>6){
		 ...
		 if(sunNum>0){
			 tip+= zuan+"<em>："+sunNum+"个</em>";
		 }
		 if(moonNum>0){
			 tip+= zhuan+"<em>："+moonNum+"个</em>";
		 }
		 if(starNum>0){
			 tip+= yb+"<em>："+starNum+"个</em>";
		 }
		 tip+="</th>";
		 arr[1] = tip;
	 }else{
...
$_sys.grade_def.push([ 50, "一等奖,二等奖,三等奖,四等奖,五等奖,六等奖,七等奖,八等奖,追加一等奖,追加二等奖,追加三等奖,追加四等奖,追加五等奖,追加六等奖,追加七等奖,,宝钻一等奖,宝钻二等奖,宝钻三等奖,宝钻四等奖"]);

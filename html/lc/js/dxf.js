    	...
    	dxf:function(data){
   		 ...
   		 var tableTpl=['',//0对阵头
	            ...
	            '<COL width="">'+
   	           '</COLGROUP>',//2对阵table控制
   	           '<tbody>'+
   	           '<tr class="{$classname}" style="display: none" dgsale="{$dsale}" lg="{$mname}" isend="1" small="{$sp0}" big="{$sp3}" pendtime="{$et}" pdate="{$itemid}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}">'+
	           ...
	           '<td style="text-align: right; border-left:1px solid #ddd;" class="h_br tdhui"><div class="dz_dv" title={$gn}><s class="s_left" style="padding-left:10px">&nbsp;</s><span class="eng b span_left"><em class="em_left" id="htid_{$itemid}">{$gn}</em><b class="b_left"> {$sp0}</b></span></div></td>'+
	           ...
	           '</tr>'+
   	           '</tbody>',//3已结束对阵-隐藏
   	           '<tbody>'+
   	           '<tr class="{$classname}" lg="{$mname}" dgsale="{$dsale}" isend="0" small="{$sp0}" big="{$sp3}" pendtime="{$et}" pdate="{$itemid}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}"  homeTeam="{$hn}" guestTeam="{$gn}">'+
   	           '<td style="cursor: pointer"><label for=m{$itemid}><input id=m{$itemid} value={$itemid}  checked type=checkbox name=m{$itemid}>{$name}</label></td>'+
   	           '<td style="background:{$cl}; color: #fff" class=league><a title="{$lmname}" href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</a></td>'+
   	           '{$dgstr}<span class="eng end_time" title="开赛时间：{$mt}">{$short_et}</span><span style="display: none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</span></td>'+
   	           '<td style="text-align: left;  cursor: pointer;border-left:1px solid #ddd" class=h_br><div class="dz_dv" style="padding-left:10px" title={$gn}><input class=chbox value=3 type=checkbox style="display: none"><span class="eng b span_right"><b class="b_right" style="margin-right:15px">{$sp3}</b> <em id="gtid_{$itemid}" class="em_right">{$gn}</em></span><s class="s_right"></s></div></td>'+
   	           '<td><STRONG class="red">{$zclose}</STRONG></td>'+  

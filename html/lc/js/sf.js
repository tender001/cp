    ...
    Class('LoadExpect',{
    	...
    	sf:function(data){
    		 ...
    		 var tableTpl=['',//0对阵头
		           ...
		           '</COLGROUP>',//2对阵table控制
		           '<tbody>'+
		           '<tr class="{$classname}" style="display: none" lg="{$mname}" dgsale="{$dsale}" isend="1" lost="{$sp0}" win="{$sp3}" pendtime="{$et}" pdate="{$itemid}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}">'+
		           ...
		           '</tbody>',//3已结束对阵-隐藏
		           '<tbody>'+
		           '<tr class="{$classname}" lg="{$mname}" isend="0" lost="{$sp0}" dgsale="{$dsale}" win="{$sp3}" pendtime="{$et}" pdate="{$itemid}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}"  homeTeam="{$hn}" guestTeam="{$gn}">'+
		           '<td style="cursor: pointer"><label for=m{$itemid}><input id=m{$itemid} value={$itemid}  checked type=checkbox name=m{$itemid}>{$name}</label></td>'+
		           '<td style="background:{$cl}; color: #fff" class=league><a title="{$lmname}" href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</a></td>'+
		           '{$dgstr}<span class="eng end_time" title="开赛时间：{$mt}">{$short_et}</span><span style="display:none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</span></td>'+

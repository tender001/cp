...
//                    div.hide();
            ...
            Y.get('#vsTable tr[isend=0]').hover(function (e, Y){//鼠标滑入滑出效果
                ...
                this.childNodes[8].style.backgroundColor = '#fffee6';
    ...
    Class('LoadExpect',{
    	...
    	spf:function(data){
    		 ...
    		 var tableTpl=['',//0对阵头
		           ...
		           '<td style="text-align: left; " class="h_br tdhui"><div class="dz_dv" title="{$gn}" style="padding-left:10px"><span class="eng b span_right"><b class="b_right1">{$sp0}</b> <em class="em_right" id="gtid_{$itemid}">{$gn}</em></span><s class="s_right"></s></div></td><td>&nbsp;</td>'+
		           '<td>'+
    ...
    Class('Buy', {
        index:function (){
            ...
            this.get('#gobuy,#gohm,#gofilter').click(function (e, Y){//提交
                var data = Y.C('choose_data'), minNum = 1, ishm = this.id == 'gohm' ? 1 : 0,
            ...
            this.get('#jjyh').click(function (e, Y){
            	var data = Y.C('choose_data'), minNum = 1,
                        ggData =  Y.C('-all-gg-type'),
                        duoc = ggData.dc;
                    if (data.length < minNum) {
                        return Y.alert('您好，请至少选择' + minNum + '场比赛!');
                    ...
                    Y.checkLimitCode(data, chkgroup, function (){
                        ...
                        Y.get("#jjyh_form").attr("action", '/jc/jjyh_rq.html');
...
//过关方式
    Class('GgType', {
        ...
        index:function (){
			...
			//少于两场的提示
			if (!isbf) {//比分不加提示
				this.lt2_info = this.get('<div id="vslt2"></div>').insert('#ggListFree', 'prev');
			}
        },

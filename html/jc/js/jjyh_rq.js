...
Class( {
    ...
    LoadDuiZhen : function(c,m,t,gg) {
		this.ajax({
					...
					type : "POST",
					dataType : "json",
					data : "pid=rspf&codes="+c+"&muli="+m + "&type=" + t + "&pnum=" + gg,
					end : function(d) {
						var obj = eval("(" + d.text + ")");
...
//		betStyleObj = "";
	...
	getduizhen:function(data){
		 ...
		 var tableTp1=['<tr teamval="{$itemid}">'+
			           '<td>{$name}</td>'+
			           '<td class="{$c3} jjyh_dz" data-val="3"><span class="yh_s_ftl">{$hn}{$close}</span><span class="yh_s_ftr">{$sp3}</span></td>'+

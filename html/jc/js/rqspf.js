...
//                });
            ...
            Y.use('mask', function (){
    			Y.get('#losehelp,#dshelp').setStyle('zIndex', 1).tip('data-help', 1, false, 360);// 帮助说明
    		});
    ...
    Class('LoadExpect',{
    	...
    	spf:function(data){
    		 ...
    		 var tableTpl=['<table width="100%" border="0" cellspacing="0" cellpadding="0" class="dc_table">'+
		           ...
		           '<td style="text-align: right; padding-right: 10px;" class=h_br><div class="dz_dv" title="{$hn}"><s class="s_left"></s><span class="eng b span_left"><em class="em_left" id="htid_{$itemid}">{$hn}</em>{$closestr} <b class="b_left1">{$sp3}</b></span></div></td>'+
		           '<td style="text-align: center;" class=h_br><span class="eng b">{$sp1}</span></td>'+
		           '<td style="text-align: left; padding-left: 10px;" class=h_br><div class="dz_dv" title="{$gn}"><span class="eng b span_right"><b class="b_right1">{$sp0}</b> <em class="em_right" id="gtid_{$itemid}">{$gn}</em></span><s class="s_right"></s></div></td><td></td>'+
		           '<td>'+
		           ...
		           '<td style="background:{$cl}; color: #fff" class=league><a title="{$lmname}" href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</a></td>'+
		           '<td><span class="eng end_time" title="开赛时间：{$mt}">{$short_et}</span><span style="display: none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</span></td>'+
		           '<td style="text-align: right; padding-right: 10px; cursor: pointer" class=h_br><div class="dz_dv" title="{$hn}"><input class=chbox value=3 type=checkbox style="display: none"><s class="s_left">&nbsp;</s><span class="eng b span_left"><em class="em_left" id="htid_{$itemid}">{$hn}</em>{$closestr} <b class="b_left1">{$sp3}</span></div></td>'+
		           ...
		           '<td style="text-align: left; padding-left: 10px; cursor: pointer" class=h_br><div class="dz_dv" title="{$gn}"><input class=chbox value=0 type=checkbox style="display: none"><span class="eng b span_right"><b class="b_right1">{$sp0}</b> <em class="em_right" id="gtid_{$itemid}">{$gn}</em></span><s class="s_right">&nbsp;</s></div></td><td style="cursor: pointer;border-right:1px solid #ddd"><a href="javascript:void(0);" class="jcq_q">全</a><input style="display: none" type="checkbox" all="0" value = "all"/></td>'+
		           '<td>'+
            ...
            this.get('#jjyh').click(function (e, Y){
            	var data = Y.C('choose_data'), minNum = Y.C('_isgg') ? 2 : 1,
                    ...
                    Y.checkLimitCode(data, chkgroup, function (){
                        ...
                        Y.get("#jjyh_form").attr("action", 'jjyh_rq.html');

    ...
    Class('LoadExpect',{
    	...
    	cbf:function(data){
    		 ...
    		 var tableTpl=['',//0对阵头
               ...
               '<TBODY>'+
               '<TR style="DISPLAY:{$display}" class="{$classname}" dgsale="{$dsale}" lg="{$mname}" isend="{$end}" odds="{$ccbf}" pdate="{$itemid}" pendtime="{$enddate}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}">'+
...
//引导启动类
    Class({
        ...
        xfhb:function(projid) {
        	...
        	$("#schemeTip,#bqch").show();
        	var data = $_trade.key.gid + "=" + encodeURIComponent(91) + "&" + $_trade.key.hid + "=" + encodeURIComponent(projid) + "&rnd=" + Math.random();
        	Y.ajax({

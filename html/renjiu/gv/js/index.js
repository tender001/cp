...
//		filter.params.lotname = host.split("/")[1];
		...
		$(".special_td").die().live("click",function(){
			...
			var selnum = parseInt($("#dxnum").text())+parseInt($("#shxnum").text())+parseInt($("#sxnum").text());
			if(pat.find(".special_td02").length<1&&selnum<10){
	...
	showTime : function(){
		...
		$.ajax({
			url : "/cpdata/game/81/s.json?rnd=" + Math.random(),
			type : "get",
			cache:false,
			dataType : "json",
			success : function(d) {
		...
		$(".unfold_box:visible a:gt(0)").each(function(i){
			$(this).unbind().bind("click", function () {
				...
				if(type == "jh" || type == "jj"){ //欧赔和
					...
					html.push('<select>'+creatOption(27,0)+'</select>≤ '+typename+' ≤<select>'+creatOption(27,27)+'</select>');
				}else if(type == "dd"||type == "lh"){		
					html.push('<select>'+creatOption(8,0)+'</select>≤ '+typename+' ≤<select>'+creatOption(8,8)+'</select>');
				}else if(type == "oh"){		
					...
					html.push('<select>'+creatOption(count,0)+'</select>≤ '+typename+' ≤<select>'+creatOption(count,9)+'</select>');
				}
...
var tools = {
	...
	calRJZhushu: function(){
		...
		$("#duizhen tr").each(function(){
			...
			if(len>0){
				...
				maxopArr[maxopArr.length]=maxsp;
				minopArr[minopArr.length]=minsp;
				// maxopj=maxopj*maxsp;
				// minopj=minopj*minsp;
				// maxoph=Common.fix(maxoph*1+maxsp*1);
				// minoph=Common.fix(minoph*1+minsp*1);
		...
		if(sel<9){
			...
			var maArr = maxopArr.sort(r);
			var miArr = minopArr.sort(r);
			for(var i=0;i<9;i++){
				minopj=minopj*miArr[i];
				minoph=Common.fix(minoph*1+miArr[i]*1);
			}
			for(var i=maArr.length-1;i>maArr.length-10;i--){
				maxopj=maxopj*maArr[i];
				maxoph=Common.fix(maxoph*1+maArr[i]*1);
			}
		}
		zhushu=tools.getCsCount(dtarr,9);
		$("#maxopj").text(Common.fix(maxopj));
		$("#minopj").text(Common.fix(minopj));
		$("#maxoph").text(maxoph);
		$("#minoph").text(minoph);
	...
	getCsCount:function(t,l){
		var d = [];
		var dl=d.length,
		tl=t.length;
		if(dl>=l||dl+tl<l){return 0;}
		var a=tools.C_each(t,l-dl);
		var al=a.length,
		c=0,
		at;
		for(var i=0;i<al;i++){
			if(dl>0){
				at=(a[i].toString()+","+d.toString()).split(",");
			}else{
				at=a[i].toString().split(",");
			}
			c+=tools.getArrElePro(at,at.length,1);
		}
		return c;
	},
	getArrElePro:function(a,i,n){
		var ar=[];
		ar.push(n);
		(function f(a,i,n){
			i--;
			if(i<0){return n;}
			n[0]*=a[i].toString().length;
			f(a,i,n);
		})(a,i,ar);
		return ar[0];
	},
	C_each:function(arr,num){
		var r=[];
		(function f(t,a,n){

...
Class( 'LineSelector', {
	...
	bindEvent : function() {
		...
		this.get(this.vsLine).hover( function() {
			...
			this.children[6].style.backgroundColor = '#fffee6';
			this.children[7].style.backgroundColor = '#fffee6';
...
//	ggGroupMap  : {'单关':27,'2串1':1,'2串3':2,'3串1':3,'3串4':5,'3串7':6,'4串1':7,'4串5':9,'4串11':12,'4串15':13,'5串1':14,'5串6':28,'5串16':29,'5串26':18,'5串31':19,'6串1':20,'6串7':30,'6串22':31,'6串42':32,'6串57':25,'6串63':26,'7串1':35,'8串1':36,'9串1':37,'10串1':38,'11串1':39,'12串1':40,'13串1':41,'14串1':42,'15串1':43},
	...
	ggGroup : [	[], [], [], ['3串1'], ['4串1', '4串5'], ['5串1', '5串6', '5串16'], ['6串1', '6串7', '6串22', '6串42'], ['7串1'], ['8串1'], ['9串1'], ['10串1'], ['11串1'], ['12串1'], ['13串1'], ['14串1'], ['15串1']	],
	ggGroupMap  : {'单关':27,'2串1':1,'2串3':2,'3串1':3,'3串4':5,'3串7':6,'4串1':7,'4串5':9,'4串11':12,'4串15':13,'5串1':14,'5串6':28,'5串16':29,'5串26':18,'5串31':19,'6串1':20,'6串7':30,'6串22':31,'6串42':32,'6串57':25,'6串63':26,'7串1':35,'8串1':36,'9串1':37,'10串1':38,'11串1':39,'12串1':40,'13串1':41,'14串1':42,'15串1':43},
	...
	updateGgInfo : function() {
		...
		this.ggTable.empty();
		if(max_limit<3){
			this.get("#gg_table tbody").html('<tr  style="text-align: center;padding-top: 10px;color: red;" ><td colspan="3">请至少选择3场比赛进行投注。</td></tr>');
		}else{
...
Class('LoadExpect',{
	...
	LoadDuiZhen : function(expect) {	
		...
		if(Class.config('nowexpect')!=expect){
			...
			case 46 :    //胜负过关
				url="/cpdata/match/beid/"+expect+"/sfgg.json";
//				http://www.159cai.com/cpdata/match/beid/140904/sfgg.xml
	...
	sfgg:function(data){
		...
		'<a  href="" target="_blank" style="color:#fff;">{$mtype}</a>'+
		'</td>'+
		'<td >{$mname}</td>'+
		...
		'<td class="h_br tdhui" style="text-align: right; border-left:1px solid #ddd;"><div class="dz_dv"><s class="s_left">&nbsp;</s>'+
...
//		'<td>'+
//		'<a href="" target="_blank" id="ox{$mid}">析</a>'+
//		'</td>'+
		'</tr>',//3 未过期
		'</tbody>'
		];	
...
//			row.bgColor=odds_issuc?(odds_data[i].cl):'#009900';
			...
			row.sp0=parseFloat($_sys.getsp(row.sf,"sp0"))>0?parseFloat($_sys.getsp(row.sf,"sp0")).rmb(false,2):'--';

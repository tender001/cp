...
check = function(){
	...
	if(sensale < 5){
		...
		if(Y.getInt(baodimoney/zumoney*100) < 5){
			errorinfo = "保底金额至少为总金额的5%，至少"+limitmoney+"元";
			Y.getTip().show('#baodimoney','<h5>'+errorinfo+'</h5>').setIco(7);
			return false;
		}
	}
	...
	Y.ajax({
		...
		data : data,
			...
			if (code == "0") {
				...
				$("#baodimoney").val($("#baodinum").val()*smoney);	
				$("#baodiScale").html(($("#baodimoney").val()/(nums*smoney)*100).toFixed(2)+'%');
				...
				Y.get('#baodimoney').keyup(function(){
					...
					if(baodimoney>0){
						Y.one('#baodiScale').innerHTML = (baodimoney/allmoney*100).toFixed(2)+"%";
					}
					checkfun();
				}); 

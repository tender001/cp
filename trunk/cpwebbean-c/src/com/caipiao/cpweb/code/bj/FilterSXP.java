...
public class FilterSXP extends FilterBase{

	public static final HashMap<String, String> sxpMaps = new HashMap<String, String>();
	...
	public void filter(CodeBean bean, FilterResult result) throws CodeFormatException {
		if(bean.getItemType() == CodeBean.NOITEM){	//文件不含场次(需指定场次)
			...
			pattern = Pattern
                    .compile("((\\s*\\d+\\s*:\\s*\\[\\s*\\d\\s*\\]\\s*/\\s*)*(\\s*\\d+\\s*:\\s*\\[\\s*\\d\\s*\\]\\s*))");
			...
			String tmpcode = bean.getCode().replaceAll("上单", "3");
			tmpcode = tmpcode.replaceAll("上双", "2");
			tmpcode = tmpcode.replaceAll("下单", "1");
			tmpcode = tmpcode.replaceAll("下双", "0");
			//System.out.println(tmpcode);
			pattern = Pattern.compile("(((\\s*\\d+\\s*(→|->)((\\s*\\d\\s*)|\\((\\s*\\d\\s*,)*(\\s*\\d\\s*)\\)))\\s*(,|;)\\s*)*(\\s*\\d+\\s*(→|->)((\\s*\\d\\s*)|\\((\\s*\\d\\s*,)*(\\s*\\d\\s*)\\)))\\s*)");
	...
	/**
	 * 兼容okooo格式
	 * 10→上单,11→下双,12→下单,13→下单,14→下双,15→上单	1	2	6_1
	 * 6→下双,8→下双,19→(上单,上双),29→(上单,上双)	4	8	4_1
	 * @param bean
	 * @param result
	 * @param code
	 * @param codeMaps
	...
	private void doOkooo(CodeBean bean, FilterResult result, String code, HashMap<String, String> codeMaps) throws CodeFormatException{
		...
		//tmpcode = tmpcode.replaceAll("1-1", "3");
		//tmpcode = tmpcode.replaceAll("1-0", "2");
		//tmpcode = tmpcode.replaceAll("0-1", "1");
		//tmpcode = tmpcode.replaceAll("0-0", "0");
     ...
     * @throws CodeFormatException
     */
    private void do5Millions(CodeBean bean, FilterResult result, String code, HashMap<String, String> codeMaps)
            throws CodeFormatException {
        HashMap<String, String> teamsMaps = new HashMap<String, String>();
        ...
        code += "|" + bean.getGuoguan();
        System.out.println("500wan="+code);
        result.putGglist(bean.getGuoguan());
        result.putItems(teamsMaps);
        result.addCode(code);
    }
...
//		String gg = len + "*1";
	...
	private String getCodeItem(String value, CodeBean bean) throws CodeFormatException{
		if(!sxpMaps.containsKey(value)){
			throw new CodeFormatException(-1, "处理转换后号码不符合投注要求", bean.getCode());
		}
		return sxpMaps.get(value);
	}
	
    public static void main(String[] args) {
		
//		List<String> nlist = new ArrayList<String>();
//		nlist.add("1,2,1,0,3,1");
//		nlist.add("1-2-1-0-3-1");
//		nlist.add("121031");
//		nlist.add("1 2 1 0 3 1");
//		nlist.add("1,2,1,*,0,3,*,*,1,*,*");
//		nlist.add("121#03##1");
		...
		ylist.add("1→上单,2→下双,3→下单,4→下单,5→下双,6→上单");//1→上单,2→下双,3→下单,4→下单,5→下双,6→上单
		ylist.add("1:[1]/2:[2]/3:[1]/4:[0]/5:[3]/6:[1]");//1:[1]/2:[2]/3:[1]/4:[0]/5:[3]/6:[1]
		ylist.add("1→3,2→3,3→1,4→0,5→3,6→1");//1→3,2→3,3→1,4→0,5→3,6→1
		
		FilterResult fr = new FilterResult();
		try {
			FilterSXP sxp = new FilterSXP();
			CodeBean bean = new CodeBean();
			bean.setLottype(88);
			bean.setPlaytype("SXP");
			bean.setCodeitems("3=上单,2=上双,1=下单,0=下双");

...
CP.Util = {
    ...
    dateDiff: function(sDate1, sDate2) {
        ...
        iDays = parseInt(Math.abs(oDate1 - oDate2) / 1e3 / 60 / 60 / 24);
        return iDays
    },
    ...
    joinString: function(gid, opt) {
        ...
        opt_ = $.map(opt_,
        function(item) {
            if (gid == "01" || gid == "50" || gid == "07" || gid == "51" || gid == "52") {
                return item += ":1:1"
        ...
        l = codes.length; i < l; i++) {
            if (lotid == 90 || lotid == 91 || lotid == 92 || lotid == 93 || lotid == 72 || lotid == 85 || lotid == 86 || lotid == 87 || lotid == 88 || lotid == 89) {
                ...
                if (lotid == "01" || lotid == "50") {
                    ...
                    case "09":
                        tmp = tmp.replace("01", "A23").replace("02", "234").replace("03", "345").replace("04", "456").replace("05", "567").replace("06", "678").replace("07", "789").replace("08", "8910").replace("09", "910J").replace("10", "10JQ").replace("11", "JQK").replace("12", "QKA");
                        break;
                    case "10":
                        tmp = tmp.replace("01", "AAA").replace("02", "222").replace("03", "333").replace("04", "444").replace("05", "555").replace("06", "666").replace("07", "777").replace("08", "888").replace("09", "999").replace("10", "101010").replace("11", "JJJ").replace("12", "QQQ").replace("13", "KKK");
                        break;
                    case "11":
                        tmp = tmp.replace("01", "AA").replace("02", "22").replace("03", "33").replace("04", "44").replace("05", "55").replace("06", "66").replace("07", "77").replace("08", "88").replace("09", "99").replace("10", "1010").replace("11", "JJ").replace("12", "QQ").replace("13", "KK");
                        break;
                    case "12":
                        tmp = "同花包选";
                        break;
    ...
    matchopencode: function(lotid, pm, cm, cd) {
        var rc = "",
        html = "";
...
CP.User = {
    ...
    info: function(fn) {
        var t = {};
        $.ajax({
            url: CP.Data.apps + "/phpu/q.phpx?fid=u_ainfo",
            dataType:'json',
            success: function(d) {
...
CP.Cookie = {
    set: function(name, value, domain, path, hour) {
        if (hour) {
            var now = new Date;
            var expire = new Date;
            expire.setTime(parseFloat(now.getTime()) + 36e5 * hour)
        }
...
$.fn.NumInput = function(options) {
    ...
    this.each(function() {
        ...
        $up.length > 0 && $up.on(start_ev,
        function() {
            doAdd()
        });
        $down.length > 0 && $down.on(start_ev,
        function() {
            doMinus()
        });
        var isPad = function() {
...
var KeyBoard_ = $('<div class="keyboard" style="top:-5rem"><o style="left:50%"></o><div><u v="vku">1</u><u v="vku">2</u><u v="vku">3</u><u v="vku">4</u><u v="vku">5</u><u v="vku">6</u></div>' + '<div><u v="vku">7</u><u v="vku">8</u><u v="vku">9</u><u v="vku">0</u><u v=vkClear><del></del></u><u v=vkOK>确认</u></div></div>');
        ...
        $ipt.Touch({
            fun: function(event) {
                if ($(this).parent().find(".keyboard").length) {
                    ...
                    $ipt.parent().find("u[v=vkOK]").bind("click",
                    function() {
                        if ($ipt.val() == "" || $ipt.val() == "0") {
                            alert("不可为空哟！");
                            $ipt.val("1");
...
CP.Popup = {
    buybox: function(options) {
        ...
        if (o.usermoney >= o.cMoney) {
            tag = false;
            $("#buy_box .zfTrue a:eq(1)").hide().siblings().show()
        } else {
            $("#buy_box .zfTrue a:eq(2)").hide().siblings().show()
        }
        $("#buy_box").removeClass("zfpopCur");
        $("#buy_reveal p:eq(0) cite").html(o.cMoney + "元");
        if (o.bonus) {
            $("#buy_reveal p:eq(1) cite").html(o.bonus + "元")
        } else {
            $("#buy_reveal p:eq(1)").hide()
        }
        if (o.ipacketmoney == "0" || o.payPara.orderType == "zh") {
            $("#buy_reveal .popuseRed span").hide()
        } else {
            $("#buy_reveal .popuseRed span").show()
        }
        $("#buy_reveal p:eq(2) cite").html(o.usermoney + "元");
        $("#buy_box,#mask").show();
        $("#buy_hide").html("");
        $("#buy_box").animate({
            "margin-top": "-" + $("#buy_box").height() / 2 + "px"
        });
        $("#buy_box").off("click.pop").on("click.pop", ".zfTrue a:eq(0)",
        function() {
            $("#buy_box,#mask").hide()
        }).on("click.pop", ".zfTrue a:eq(1)",
        function() {
            window.location.href = "#type=url&p=user/charge.html";
            $("#buy_box,#mask").hide()
        }).on("click.pop", ".zfTrue a:eq(2)",
        function() {
            o.cupacketid = "";
            o.redpacket_money = "";
        ...
        }).on("click.pop", "#buy_reveal div span",
        function() {
                    ...
                    success: function(xml) {
                        ...
                        if (r.length) {
                            ...
                            r.each(function(i) {
                                ...
                                var crpname = $(this).attr("crpname");
                                var irmoney = $(this).attr("irmoney");
                                var cddate = $(this).attr("cddate");
                                var kymoney = $(this).attr("kymoney");
                                kymoney = kymoney || "0";
        ...
        }).delegate("#buy_hide .pdLeft1", "click.pop",
        function() {
...
CP.Pay = function() {
    var init = function(opt) {
        ...
        if (opt_.hid) {
            ...
            } else if (opt.payPara.orderType == "zh") {
                data = {
                    gid: opt.gid,
                    pid: opt_.pid,
                    codes: opt_.codes,
                    mulitys: opt_.muli,
                    zflag: opt.payPara.zflag,
                    ischase: "1",
                    money: opt_.countMoney,
                    source: CP.Data.source,
                    upay: ""
...
window["alert"] = function(msg, loading) {
    if (!loading) {
        ...
        $(".alertBox").css({
            webkitAnimationName: "boxfade_loading",
            opacity: .8
        });
        $("#mask").show()
    }
...
var D = {
    ...
    confirm: function(msg, fn, tag) {
        ...
        $("#mask, #dConfirm").show();
        $("#dConfirm a:eq(0)").one("click",
        function() {
            $("#dConfirm,#mask").hide()
        });
        $("#dConfirm a:eq(1)").one("click",
        function() {
            if (typeof fn == "function") {
                fn()
            }
...
try {
    if (window.console && window.console.log) {
        if ( !! window.ActiveXObject || "ActiveXObject" in window) {
            ...
            console.log("%cHello world", "color:#347be4;font-size:25px;-webkit-text-fill-color:#347be4;-webkit-text-stroke: 1px black;")
        }
        console.log("\n")
    }
} catch(b) {}
...
var TopAnch = function() {
    ...
    var init = function(obj) {
        if (!obj) {
            ...
            if (obj.isScore) {
                $dom.score.show();
                $dom.score.off().on(end_ev,
                function() {
                    location.href = confTemp.isScore
                })
            } else {
                $dom.score.hide()
            }
            ...
            if (obj.headerSelect) {
                ...
                for (var i = 0,
                l = obj.headerSelect.length; i < l; i++) {
...
//                })
    ...
    var openLogin = function(xo) { ! xo && (url_ = false) || (url_ = xo);
        pageLoading.hide();
        $("body").removeClass("home");
        $("#home_page, #content_home").hide();
        $("#login").show();
        $("#btn_next").attr("class","RfcbackHome")
        var user_name = localStorage.getItem("username"); !! user_name && $("#username").val(user_name).next().show();
        TopAnch.init({
            title: "登录",
            prevShow: true,
            prevFun: function() {
                if ($("#mylottery").length || $("#operation").length) {
        ...
        $("#login .s3 input").off().bind("keyup",
        function() {
            $(this).val() != "" && $(this).next().show() || $(this).next().hide()
        });
        $("#login .s3 em").off().bind("click",
        function() {
            $(this).hide().prev().val("")
        })
    };
...
var hashchange = function() {
    ...
    window.pageRoute = decodeURIComponent(CP.Cookie.get("pageRoute")) || "";
    var getPage = function(url) {
        ...
        if ($("#home_page").length && url == "index.html") {
            ...
            if (url.indexOf("jczq") > 0 || url.indexOf("jcdg") > 0 || url.indexOf("jclq") > 0 || url.indexOf("bjdc") > 0 || url.indexOf("sfgg") > 0 || url.indexOf("r9") >= 0 || url.indexOf("sfc") >= 0 || url.indexOf("gddg") >= 0) {

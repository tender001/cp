...
Class('LoadExpect',{
	...
	bqc:function(data){
		...
		r.each(function(row,i){
			...
			row.sp33=$_sys.getsp(row.bqc,"33")?parseFloat($_sys.getsp(row.bqc,"33")).rmb(false,2):'--';
			row.sp31=$_sys.getsp(row.bqc,"31")?parseFloat($_sys.getsp(row.bqc,"31")).rmb(false,2):'--';
			row.sp30=$_sys.getsp(row.bqc,"30")?parseFloat($_sys.getsp(row.bqc,"30")).rmb(false,2):'--';

			row.sp13=$_sys.getsp(row.bqc,"13")?parseFloat($_sys.getsp(row.bqc,"13")).rmb(false,2):'--';
			row.sp11=$_sys.getsp(row.bqc,"11")?parseFloat($_sys.getsp(row.bqc,"11")).rmb(false,2):'--';
			row.sp10=$_sys.getsp(row.bqc,"10")?parseFloat($_sys.getsp(row.bqc,"10")).rmb(false,2):'--';			
				
			row.sp03=$_sys.getsp(row.bqc,"03")?parseFloat($_sys.getsp(row.bqc,"03")).rmb(false,2):'--';
			row.sp01=$_sys.getsp(row.bqc,"01")?parseFloat($_sys.getsp(row.bqc,"01")).rmb(false,2):'--';
			row.sp00=$_sys.getsp(row.bqc,"00")?parseFloat($_sys.getsp(row.bqc,"00")).rmb(false,2):'--';			
			
			row.rz=$_sys.getrz(row.rs,2);//this.getsp(row.bqc,"rz");
			row.rs=parseFloat($_sys.getrs(row.rs,2)).rmb(false,2);//parseFloat(this.getsp(row.bqc,"rs")).rmb(false,2);
			...
			if (out_of_date&&row.rz!=''){//已经开出赛果的&&row.bf!=''
				...
				row.sp30str=(row.rz=='30'?('<span class="red">'+row.rs+'</span>'):'--');

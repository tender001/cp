...
//                    var div = Y.get(this).parent('tr').find(".dz_dv");
//                    div.hide();
                    tr.hide();
                    ...
                    if (chk = Y.one('input', this, true)) {//hide error
                        ...
                        if (isAll) {//如果是全选
                            ...
                            chks.slice(0,-1).each(function (el){
                                ...
                                if (el.checked) {
                                    scm.push(el.value);
                                    td.removeClass("h_brx");
                                    td.addClass(hotCss);
//                                    td.attr("class","h_br "+hotCss);
                                    me.parent('tr').find('.jcq_q').slice(0).html("清");
                                }else{
                                    td.removeClass(hotCss);
                                    me.parent('tr').find('.jcq_q').slice(0).html("全");
                                }
                            });
                         }else{
            ...
            Y.get('#vsTable tr[mid]').hover(function (e, Y){//鼠标滑入滑出效果
                ...
                this.childNodes[3].className=this.childNodes[3].childNodes[0].childNodes[0].checked==true?"h_br label_cd":"h_br";
                this.childNodes[4].className=this.childNodes[4].childNodes[0].childNodes[0].checked==true?"h_br label_cd":"h_br";
                 ...
                 if (vsTr) {
                     ...
                     chkbox.find(':checkbox').slice(this.isbf ? 0 : 1).each(function (chk){
                         ...
                         if (chk.checked) {
                             ...
                             td.parent('tr').find('.jcq_q').slice(0).html("清");
                         }else{
                             td.removeClass(this.ns.hotCss);
                             td.parent('tr').find('.jcq_q').slice(0).html("全");
                           
                         }
                     });
                     this.get(vsTr).show();
//                     this.get(vsTr).find(".dz_dv").show();
...
//选中对阵表
    Class('Choose', {
        ...
        addLive: function (){
//        	 var Y=this,tag = this.ptype.val().indexOf('spf')>-1 ? '':'>a';
        	var Y=this;
        ...
        addChoose: function (chk, mid, vstr){
            ...
            if (tr.size() == 0) {
                ...
                var A = vstr.cells[3].getElementsByTagName('em')[0].innerHTML,
                    B = vstr.cells[4].getElementsByTagName('em')[0].innerHTML,
...
//                sp = [vstr.getAttribute('win'), vstr.getAttribute('draw'), vstr.getAttribute('lost')];['rqspf3','spf0','spf3','rqspf0']);
            	
                sp = [vstr.getAttribute('rsp3'),vstr.getAttribute('sp0'),vstr.getAttribute('sp3'), vstr.getAttribute('rsp0')];
            }else{
...
//加载 期次-对阵
    Class('LoadExpect',{
    	...
    	LoadDuiZhen : function() {	
    		...
    		this.ajax({
    			        url: "/cpdata/match/jczq/jczq_rspf.json?v=" + Math.random(),
    			        cache:false,
    					end : function(data) {
...
//		           '<td style="text-align: center;" class=h_br><span class="eng b">{$sp1}</span></td>'+
		           ...
		           '</tbody>',//已结束对阵
		           '<tbody>'+
		           '<tr class="{$classname}" lg="{$mname}" isend="0"  sp0="{$sp0}" sp3="{$sp3}" rsp0="{$rsp0}" rsp3="{$rsp3}" pendtime="{$et}" rq="{$close}" pdate="{$itemid}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}">'+
		           '<td style="cursor: pointer"><label for=m{$itemid}><input id=m{$itemid} value={$itemid}  checked type=checkbox name=m{$itemid}>{$name}</label></td>'+
		           '<td style="background:{$cl}; color: #fff" class=league><a title="{$lmname}" href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</a></td>'+
		           '<td><span class="eng end_time" title="开赛时间：{$mt}">{$short_et}</span><span style="display: none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</span></td>'+
...
//		           '<td style="text-align: center; cursor: pointer" class=h_br><input class=chbox value=1 type=checkbox style="display: none" ><span class="eng b" >{$sp1}</span></td>'+
			...
			r.each(function(row,i){
    			...
    			var spfstr=row.spf.split(",");
    			row.sp3=(spfstr[0]!=''?(parseFloat(spfstr[0]).rmb(false,2)):'--');
    			row.sp1=(spfstr[1]!=''?(parseFloat(spfstr[1]).rmb(false,2)):'--');
    			row.sp0=(spfstr[2]!=''?(parseFloat(spfstr[2]).rmb(false,2)):'--');
    			var rspfstr=row.rspf.split(",");
    			row.rsp3=(rspfstr[0]!=''?(parseFloat(rspfstr[0]).rmb(false,2)):'--');
    			row.rsp1=(rspfstr[1]!=''?(parseFloat(rspfstr[1]).rmb(false,2)):'--');
    			row.rsp0=(rspfstr[2]!=''?(parseFloat(rspfstr[2]).rmb(false,2)):'--');
				row.sid=row.sid;
				row.gtid=row.gtid;
    			all_matches++;		
    						
    			if (parseInt(row.close)==-1){
    				row.leftstr='<b class="exy_w exy_l">胜</b><b class="b_left">'+row.sp3+'</b>';
    				row.rightstr='<b class="exy_w">不败</b>';
    				row.rightsp='<b class="b_right">'+row.rsp0+'</b>';
    				row.lefttype="spf";
    				row.righttype="rqspf";
    				row.leftvalue="spf3";
    				row.rightvalue="rqspf0";
    			}else if (parseInt(row.close)==1){
    				row.leftstr='<b class="exy_w">不败</b><b class="b_left">'+row.rsp3+'</b>';
    				row.rightstr='<b class="exy_w exy_l">胜</b>';
    				row.rightsp='<b class="b_right">'+row.sp0+'</b>';
    				row.lefttype="rqspf";
    				row.righttype="spf";
    				row.leftvalue="rqspf3";
    				row.rightvalue="spf0";
    			}else{
    				row.rightstr="";
    				row.leftstr="";
  ...
  //发起购买
    Class('Buy', {
        index:function (){
            ...
            this.get('#gobuy,#gohm,#gofilter').click(function (e, Y){//提交
                ...
                Y.checkLimitCode(data, chkgroup, function (){
                    ...
                    Y.get('#codes').val(data.map(function (a){
                    	//48819|131118001|SPF>[3]/48819|131118001|RSPF>[0]/48820|131118002|RSPF>[3]
                    	//48819|131118001[3,1,0]/48820|131118002[3,1,0]/48821|131118003[3,1,0]
                    	//['rqspf3','spf0','spf3','rqspf0']
                    	var betstr="";
                    	var betstr2="";
                    	if(Y._id2Name(a.data)[0]=="rqspf3"){
                    		betstr="RSPF>[3]";
                    	}else if(Y._id2Name(a.data)[0]=="rqspf0"){
                    		betstr="RSPF>[0]";
                    	}else if(Y._id2Name(a.data)[0]=="spf3"){
                    		betstr="SPF>[3]";
                    	}else if(Y._id2Name(a.data)[0]=="spf0"){
                    		betstr="SPF>[0]";
                    	}
                    	 var c = a.mid+'|'+a.pname+'|'+betstr;//格式
                    	if(Y._id2Name(a.data)[1]=="rqspf3" ||Y._id2Name(a.data)[1]=="rqspf0"||Y._id2Name(a.data)[1]=="spf3"||Y._id2Name(a.data)[1]=="spf0"){
                    		if(Y._id2Name(a.data)[1]=="rqspf3"){
                    			betstr2='/'+a.mid+'|'+a.pname+'|RSPF>[0]';
                         	}else if(Y._id2Name(a.data)[1]=="rqspf0"){
                         		betstr2='/'+a.mid+'|'+a.pname+'|RSPF>[0]';
                         	}else if(Y._id2Name(a.data)[1]=="spf3"){
                         		betstr2='/'+a.mid+'|'+a.pname+'|SPF>[0]';
                         	}else if(Y._id2Name(a.data)[1]=="spf0"){
                         		betstr2='/'+a.mid+'|'+a.pname+'|SPF>[0]';
                         	}
...
//                        Y.openUrl('jj_yh.html?codes='+Y.get('#codes').val()+'&totalmoney='+totalmoney+'&muli='+Y.get('#bs').val(),1000,650);
                    });
           });
    ...
    Class({
    		...
    		this.ajax({
    				url:"/cpdata/omi/jczq/odds/odds.xml",
                         ...
                         this.qXml('//row', data.xml, function (u, i){
                        	    Y.get("#mn"+u.items.xid).attr("href","http://info.gucheng.159cai.com/index.php?controller=main&lid="+u.items.lid+"&sid="+u.items.sid+"&cid="+u.items.cid+"&t="+u.items.t);
                        	    Y.get("#ox"+u.items.xid).attr("href","http://info.gucheng.159cai.com/index.php?controller=analysis&action=index&mid="+u.items.oddsmid+"&sit=4&lotyid=6");
                        	    Y.get("#oz"+u.items.xid).attr("href","http://odds.gucheng.159cai.com/index.php?controller=detail&action=index&mid="+u.items.oddsmid+"&sit=1&lotyid=6");
                        	    Y.get("#oy"+u.items.xid).attr("href","http://odds.gucheng.159cai.com/index.php?controller=detail&action=index&mid="+u.items.oddsmid+"&sit=2&lotyid=6");
	            				...
	            				$("#htid_"+u.items.xid).attr("data",u.items.htid).parent().prev().html(hm==""|| typeof hm == undefined?"&nbsp;":'['+hm+']');
	            				$("#gtid_"+u.items.xid).attr("data",u.items.gtid).parent().next().html(am=="" || typeof am == undefined?"&nbsp;":'['+am+']');	
                				if(lottype==34 || lottype==42){//添加赔率信息
                					Y.get("#oh"+u.items.xid).html(parseFloat(u.items.oh+"00").rmb(false,2));
                					Y.get("#od"+u.items.xid).html(parseFloat(u.items.od+"00").rmb(false,2));
                					Y.get("#oa"+u.items.xid).html(parseFloat(u.items.oa+"00").rmb(false,2));
                				}
                         });  
						   if(this.oneodds){
                             var xhhistory = "";
        ...
        _index:function (){
            ...
            default://spf
                ...
                this.C('optKeys', ['rqspf3','spf0','spf3','rqspf0']);
                this.C('spXml', 1);

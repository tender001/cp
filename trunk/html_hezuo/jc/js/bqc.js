    ...
    Class('LoadExpect',{
    	...
    	bqc:function(data){
   		 ...
   		 var tableTpl=['<TABLE class=dc_table border=0 cellSpacing=0 cellPadding=0 width="100%">'+
              '<COLGROUP><COL width=70><COL width=60><COL width=50><COL width=177><COL width=38>'+
              '<COL width=38><COL width=38><COL width=38><COL width=38><COL width=38><COL width=38><COL width=38><COL width=38><COL width=38></COLGROUP>'+
              '<TBODY>'+
              '<TR><TH>赛事编号</TH><TH>赛事类型</TH>'+
              '<TH><SELECT id=select_time><OPTION selected value=0>截止</OPTION> <OPTION value=1>开赛</OPTION></SELECT></TH>'+
              '<TH>主队 VS 客队</TH><TH>胜-胜</TH><TH>胜-平</TH><TH>胜-负</TH>'+
              '<TH>平-胜</TH><TH>平-平</TH><TH>平-负</TH><TH>负-胜</TH><TH>负-平</TH>'+
              '<TH>负-负</TH><TH>数据</TH></TR></TBODY></TABLE>',//0对阵头
              ...
              '<COLGROUP><COL width=70><COL width=60><COL width=50><COL width=82><COL width=4><COL width=81><COL width=38>'+
              ...
              '<TBODY>'+
              '<TR class="{$classname}" style="DISPLAY: none" lg="{$mname}" isend="1" odds="{$bqc}" pdate="{$itemid}" pendtime="{$et}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}">'+
              ...
              '</TBODY>',//3隐藏对阵
              '<TBODY>'+
              '<TR lg="{$mname}" class="{$classname}" isend="0" odds="{$bqc}" pdate="{$itemid}" pendtime="{$et}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}">'+
              '<TD style="CURSOR: pointer"><LABEL for=m{$itemid}><INPUT id=m{$itemid} class=chbox value={$itemid} CHECKED type=checkbox name=m{$itemid}>{$name}</LABEL></TD>'+
              '<TD style="BACKGROUND:{$cl}; COLOR: #fff" class=league><A title={$lmname} href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</A></TD>'+
              '<TD><SPAN class="eng end_time" title="开赛时间：{$mt}">{$short_et}</SPAN><SPAN style="DISPLAY: none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</SPAN> </TD>'+
   		...
   		var obj = eval("(" + data.text + ")");
			...
			r.each(function(row,i){
   			...
   			if (Y.getDate(data.date)>Y.getDate(row.et)){//已经过期的场次
   				out_of_date_matches++;
   				row.shuju='<a href="http://odds.caipiao.gucheng.com/fenxi/shuju.php?id=318510&show=2" target="_blank">析</a> <a href="http://odds.caipiao.gucheng.com/fenxi/yazhi.php?id=318510&show=2" target="_blank">亚</a> <a href="http://odds.caipiao.gucheng.com/fenxi/ouzhi.php?id=318510&show=2" target="_blank">欧</a>';

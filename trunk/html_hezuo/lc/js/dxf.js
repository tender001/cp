    ...
    Class('Vs', {
        ...
        _init: function (){
            ...
            if (this.isbf) {// 显隐明细行按钮
                this.get('#vsTable').live('a.bf_btn', 'click', function (e, Y){
                    ...
                    if (this.className.indexOf('public_Lblue') > -1) {// now
																		// is
																		// off
		...
		_upSp: function (arrSp){
            ...
            function upLine(tr, xml_m, map){
                ...
                spInputs.each(function (input, j){// 遍历check查找对应的xml
													// value，更新sp 值
...
// 选中对阵表
    Class('Choose', {
        ...
        addMsg: function (){
            this.onMsg('msg_vs_change', function (tr, chkBox, clip){// tr: 数据源,
																	// chkBox:
																	// 复选框所在位置,
																	// bf可能有多行tr,
																	// clip:
...
// 加载购买对阵
    Class('LoadExpect',{
    	...
    	LoadDuiZhen : function() {
    		...
    		this.ajax({
    			        url: "/cpdata/match/jclq/"+uri+".json?rnd=" + Math.random(),
    			        cache:false,
    					end : function(data) {
    	...
    	dxf:function(data){
   		 ...
   		 var tableTpl=['<TABLE class=dc_table border=0 cellSpacing=0 cellPadding=0 width="100%">'+
                  ...
                  '<TR>'+
                    ...
                    '<TH>客队(<b class="red">大分</b>)</TH>'+
                    '<TH>预设总分</TH>'+
                    '<TH>主队(<b style="color:blue">小分</b>)</TH>'+
                    '<TH>平均欧赔</TH><TH>数据</TH></TR></TBODY></TABLE>',// 0对阵头
	           ...
	           '<td style="text-align: right; padding-right: 10px;" class=h_br><div class="dz_dv" title={$gn}><s class="s_left">&nbsp;</s><span class="eng b span_left"><em class="em_left" id="htid_{$itemid}">{$gn}</em><b class="b_left"> {$sp0}</b></span></div></td>'+
	           '<td><STRONG class="red">{$zclose}</STRONG></td>'+
	           '<td style="text-align: left; padding-left: 18px;" class=h_br><div class="dz_dv" title={$hn}><span class="eng b span_left"><b class="b_left">{$sp3}<b> <em class="em_left" id="gtid_{$itemid}">{$hn}</em></span><s class="s_right"></s></div></td>'+
	           '<td>'+
	           ...
	           '</tr>'+
   	           '</tbody>',// 3已结束对阵-隐藏
   	           '<tbody>'+
   	           '<tr class="{$classname}" lg="{$mname}" isend="0" small="{$sp0}" big="{$sp3}" pendtime="{$et}" pdate="{$itemid}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}"  homeTeam="{$hn}" guestTeam="{$gn}">'+
   	           '<td style="cursor: pointer"><label for=m{$itemid}><input id=m{$itemid} value={$itemid}  checked type=checkbox name=m{$itemid}>{$name}</label></td>'+
   	           '<td style="background:{$cl}; color: #fff" class=league><a title="{$lmname}" href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</a></td>'+
   	           '<td><span class="eng end_time" title="开赛时间：{$mt}">{$short_et}</span><span style="display: none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</span></td>'+
   	           '<td style="text-align: left; padding-left: 18px; cursor: pointer" class=h_br><div class="dz_dv" title={$gn}><input class=chbox value=3 type=checkbox style="display: none"><span class="eng b span_right"><b class="b_right">{$sp3}</b> <em id="gtid_{$itemid}" class="em_right">{$gn}</em></span><s class="s_right"></s></div></td>'+
   	           '<td><STRONG class="red">{$zclose}</STRONG></td>'+  
	        	'<td style="text-align: right; padding-right: 18px; cursor: pointer" class=h_br><div class="dz_dv" title={$hn}><input class=chbox value=0 type=checkbox style="display: none"><s class="s_left">&nbsp;</s><span class="eng b span_left"><em id="htid_{$itemid}" class="em_left">{$hn}</em> <b class="b_left">{$sp0}</b></span></div></td>'+
   	           '<td>'+
   		...
   		var obj = eval("(" + data.text + ")");
			...
			row.classname=i%2==0?"even odd":"even";
   			...
   			var spstr=row.dxf.split(",");
   			row.sp3=(spstr[0]!=''?(parseFloat(spstr[0]).rmb(false,2)):'--');
   			row.sp0=(spstr[1]!=''?(parseFloat(spstr[1]).rmb(false,2)):'--');
   			...
   			if (Y.getDate(data.date)>Y.getDate(row.et)){// 已经过期的场次
   				out_of_date_matches++;
   					row.shuju='<a href="#">析</a> <a href="#">欧</a>';
...
// 发起购买
    Class('Buy', {
        index:function (){
            ...
            this.onMsg('msg_choose_update', function (data){// 选择场次或者子项变化,
															// 可能化引起过关方式自动切换
                this._upView(data)
            });
...
// 过关计算注数
    (function (){
        ...
        Class.extend('getGgZs', function(arr/* intArray */,ggType) {// [3,2,2,1]选赛果个数,
																	// 'n串m'
            switch (ggType){
    		...
    		this.ajax({
    				url:"/cpdata/omi/jclq/odds/odds.xml",
                         ...
                         this.qXml('//row', data.xml, function (u, i){
                        	 Y.get("#mn"+u.items.xid).attr("href","http://info.caipiao.gucheng.com/index.php?controller=lqmain&lid="+u.items.lid+"&sid="+u.items.sid);
                        	 Y.get("#hn"+u.items.xid).attr("href","http://info.caipiao.gucheng.com/index.php?controller=lqteam&action=index&sid=1&tid="+u.items.htid);
                        	 Y.get("#gn"+u.items.xid).attr("href","http://info.caipiao.gucheng.com/index.php?controller=lqteam&action=index&sid=1&tid="+u.items.gtid);
                        	 Y.get("#ox"+u.items.xid).attr("href","http://info.caipiao.gucheng.com/index.php?controller=fenxi&mid="+u.items.mid);
                        	 Y.get("#oz"+u.items.xid).attr("href","http://info.caipiao.gucheng.com/index.php?controller=lqeodds&mid="+u.items.mid);
            ...
            if (backData) {
                ...
                if (ggs = backData.sgtype) {
                    ...
                    ggStr.each(function (s){// 选中过关方式
                        ...
                        if (ggChk.size()) {
                            ggChk.prop('checked', false).one().click()// use
																		// form
																		// elements
																		// method
                        }                           
...
(function (){
    ...
    function get8(arr){return $CL(arr,arr.length-3)};
    function get9(arr){return $CL(arr,arr.length-4)};
    Class.extend('getDCZs', function(arr/* intArray */,ggType) {// [3,2,2,1]选赛果个数,
																// 'n串m'
        switch (ggType){
            case '2串1':case '3串1':case '4串1':case '5串1':case '6串1':case '7串1':case '8串1':case '9串1':case '10串1':
            case '11串1':case '12串1':case '13串1':case '14串1':case '15串1':return get1(arr);// n,
																							// 串1直接连乘
            case '3串4':case '4串5':case '5串6':case '6串7':case '7串8':case '8串9':return get2(arr);// n,
																								// n-1
            case '4串11':case '5串16':case '6串22':return get3(arr);// n, n-1,
																	// n-2

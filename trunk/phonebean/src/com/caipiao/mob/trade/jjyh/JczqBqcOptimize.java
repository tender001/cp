...
public class JczqBqcOptimize extends AbstractJczqOptimize {
...
//				result.put("desc", sdesc.substring(0, sdesc.length()-1));
	...
	protected String[] createItemToArray(MatchBean bean) {
		...
		for (int i=0;i<ss.length;i++) {
			String spv = getSpValue(bean.bqc,"BQC",ss[i]);
//			lst.add(bean.itemid + "=" + ss[i] + "~" + bean.name + "=" + MNAMES.get(ss[i]) + "[" +  spv + "]");
			lst.add(bean.itemid + "=" + ss[i] + "~" + bean.hn + "=" + MNAMES.get(ss[i]) + "[" +  spv + "]");
		}
	...
	public static void main(String[] args) {
		JczqBqcOptimize optimize = new JczqBqcOptimize();
		String s = optimize.optimize("BQC|130412002=3-0/1-1,130412003=0-0", 10000, 2, 0,"","","");

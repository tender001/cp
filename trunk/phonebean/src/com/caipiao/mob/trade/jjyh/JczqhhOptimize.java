...
import com.caipiao.mob.trade.jjyh.AbstractJczqOptimize.MatchBean;
...
public class JczqhhOptimize extends AbstractJczqOptimize {

	private final static HashMap<String,String> MNAMES = new HashMap<String,String>();
	static {
		MNAMES.put("SPF", "胜平负");
		MNAMES.put("RSPF", "让球胜平负");
		MNAMES.put("CBF", "猜比分");
		MNAMES.put("BQC", "半全场");		
		MNAMES.put("JQS", "进球数");
	...
	/**
	 * HH|130319001>BQC=3-3,130319003>SPF=3,130319004>JQS=1,130319005>SPF=3|4*1
	 */
	@Override
...
//				result.put("desc", sdesc.substring(0, sdesc.length()-1));
	...
	protected String[] createItemToArray(MatchBean bean) {
		...
		String sl = bean.selected;
		String[] ss = StringUtil.splitter(sl, "+");
		for (int i=0;i<ss.length;i++) {
			String[] sss = StringUtil.splitter(ss[i], "=");
			String play = sss[0];
			String sopt = sss[1];
			String[] ssss = StringUtil.splitter(sopt, "/");
			
			String sps = "";
			if ( play.equalsIgnoreCase("spf")) {
				sps = bean.spf;
			} 
			if ( play.equalsIgnoreCase("cbf")) {
				sps = bean.cbf;
			} 
			if ( play.equalsIgnoreCase("bqc")) {
				sps = bean.bqc;
			} 
			if ( play.equalsIgnoreCase("jqs")) {
				sps = bean.jqs;
			}
			if ( play.equalsIgnoreCase("rspf")) {
				sps = bean.rspf;
			} 
			
			for (int j=0;j<ssss.length;j++) {
//				System.out.println("play=" + play + " sopt=" + sopt);
				String spv = getSpValue(sps,play,ssss[j]);
				String code = play + "=" + ssss[j];
				String re = MNAMES.get(ssss[j])==null?ssss[j]:MNAMES.get(ssss[j]);
				String pname = MNAMES.get(play) + "(" +re+ ")";
//				lst.add(bean.itemid + ">" + code  + "~" + bean.name + "=" + pname + "[" +  spv + "]");
				lst.add(bean.itemid + ">" + code + "~" + bean.hn + "=" + pname + "[" +  spv + "]");
			}
		}
	...
	private String createSelectedDesc(String sopt) {
		...
		String[] ss = StringUtil.splitter(sopt, "+");
		for (int i=0;i<ss.length;i++) {
			String[] sss = StringUtil.splitter(ss[i], "=");
			sb.append(MNAMES.get(sss[0]));
			sb.append("=").append(sss[1]);
			sb.append(" ");
		}
	...
	public static void main(String[] args) throws Exception {
		JczqhhOptimize optimize = new JczqhhOptimize();
		String s = optimize.optimize("140915001>SPF=3,140915002>JQS=0/1+CBF=1:0/9:9/0:9", 10000, 2, 0,"","","");
		System.out.println(s);
		
//		System.out.println(URLEncoder.encode("130413062>SPF=3,130414041>JQS=0/1+CBF=1:0/9:9/0:9"));
	}
}

...
public class BjdcJqsOptimize extends AbstractBjdcOptimize {
...
//				result.put("desc", sdesc.substring(0, sdesc.length()-1));
	...
	protected String[] createItemToArray(MatchBean bean) {
		...
		for (int i=0;i<ss.length;i++) {
			String spv = getSpValue(bean.spv,"JQS",ss[i]);
//			lst.add(bean.mid + "=" + ss[i] + "~" + bean.mid + "=(" + ss[i] + ")[" +  spv + "]");
			lst.add(bean.mid + "=" + ss[i] + "~" +bean.hn + "=(" + ss[i] + ")[" +  spv + "]");
		}
	...
	public static void main(String[] args) {
		BjdcJqsOptimize optimize = new BjdcJqsOptimize();
		String s = optimize.optimize("JQS|53=3,54=1,55=3/1/0", 7, 2, 3,"1","89","140812");

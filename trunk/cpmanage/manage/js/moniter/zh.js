$(document).ready(function(){
	...
	initGame = function(eselect, idx){
		var zcs = SYS_CONFIG.getGameByType('kp').concat(SYS_CONFIG.getGameByType('mp'));
		var gcount = zcs.length;
					...
					Tool.table({
						...
						rowclick:function(data){
							if($(data).attr("column") == 10){
								$.confirm({
									'title'		: '追号方案监控--撤销追号',
									'message'	: '你正在操作的是撤销方案,彩种='+SYS_CONFIG.getGame($("#tbb").val())+' 追号编号='+$(fdata).attr("czhid") + "_" + $(fdata).attr("idetailid")+' 确定要执行吗?',
									'buttons'	: {
										'Yes'	: {
											'class'	: 'blue',
											'action': function(){

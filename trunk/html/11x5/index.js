...
Class.extend('exportCode', function (){
	...
	if(zid!=""&&typeof(zid) != 'undefined'){
		...
		var data = $_trade.key.gid + "=" + encodeURIComponent("54") + "&tid=" + encodeURIComponent(zid) + "&rnd=" + Math.random();
		Y.ajax({
			...
			data : data,
				...
				if (code == "0") {
					...
					var icmoney = r.icmoney===undefined?r[0].icmoney:r.icmoney;
...
//        	var pid = this.get('#play_tabs a[class=cur]').attr("value");
        	Class.C('playid', pid);
...
//		var ctp2 = ' <i style="color:red">{1}</i><i>{2}</i><i>{3}</i><i>{4}</i><i>{5}</i><i>{6}</i><i>{7}</i><i>{8}</i><i>{9}</i><i>{10}</i><i>{11}</i>';
//		$("haoma_1 div.nxyl nxyl1 i").html(ctp2.format(llcc[0],llcc[1],llcc[2],llcc[3],llcc[4]))
...
//        			this.get(li).html('<i>'+c+'</i>');
    		 ...
    		 var opentimetd = this.get('td', ul).slice(14,15);//填写开奖时间的位置

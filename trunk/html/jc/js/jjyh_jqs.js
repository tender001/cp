...
Class( {
    ...
    LoadDuiZhen : function(c,m,t,gg) {
		this.ajax({
					...
					type : "POST",
					dataType : "json",
					data : "pid=jqs&codes="+encodeURIComponent(c)+"&muli="+m + "&type=" + t + "&pnum=" + gg,
					end : function(d) {
						var obj = eval("(" + d.text + ")");
	...
	getduizhen:function(data){
		 ...
		 var tableTp1=['<tr>'+
			           '<td>{$name}</td>'+
			           '<td><span class="yh_s_ftl">{$hn}{$close}</span></td>'+
			           '<td><span class="yh_s_ftl">{$gn}</span></td>'+
			           '<td><span class="yh_s_ftl">{$sdesc}</span></td>'+
			           '<td>{$bet3}</td>'+
			           '<td>{$bet1}</td>'+
			           '<td>{$bet0}</td>'+
			           '</tr>'

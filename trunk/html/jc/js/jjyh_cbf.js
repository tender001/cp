...
$_sys.cbf.push([ 8, "4:2" ]);
$_sys.cbf.push([ 9, "5:0" ]);
$_sys.cbf.push([ 10, "5:1" ]);
$_sys.cbf.push([ 11, "5:2" ]);
$_sys.cbf.push([ 12, "9:0" ]);

$_sys.cbf.push([ 13, "0:0" ]);
$_sys.cbf.push([ 14, "1:1" ]);
$_sys.cbf.push([ 15, "2:2" ]);
$_sys.cbf.push([ 16, "3:3" ]);
$_sys.cbf.push([ 17, "9:9" ]);

$_sys.cbf.push([ 18, "0:1" ]);
$_sys.cbf.push([ 19, "0:2" ]);
$_sys.cbf.push([ 20, "1:2" ]);
$_sys.cbf.push([ 21, "0:3" ]);
$_sys.cbf.push([ 22, "1:3" ]);
$_sys.cbf.push([ 23, "2:3" ]);
$_sys.cbf.push([ 24, "0:4" ]);
$_sys.cbf.push([ 25, "1:4" ]);
$_sys.cbf.push([ 26, "2:4" ]);
$_sys.cbf.push([ 27, "0:5" ]);
$_sys.cbf.push([ 28, "1:2" ]);
$_sys.cbf.push([ 29, "2:9" ]);
$_sys.cbf.push([ 30, "0:9" ]);

$_sys.getcbfindex = function(f,n) {
	if (typeof(n)=='undefined'){n=0;};
	for ( var i = 0; i < $_sys.cbf.length; i++) {
		if ($_sys.cbf[i][1] == f) {
			return $_sys.cbf[i][n];
		}
	}
...
Class( {
    ...
    LoadDuiZhen : function(c,m,t,gg) {
		this.ajax({
					...
					type : "POST",
					dataType : "json",
					data : "pid=cbf&codes="+encodeURIComponent(c)+"&muli="+m + "&type=" + t + "&pnum=" + gg,
					end : function(d) {
						var obj = eval("(" + d.text + ")");
			...
			var jqbet=""
			 ...
			 if(jqs.split("/").length>1){
				 jqs.split("/").each(function(e,Y){
					 jqbet+='<em data-val="'+e+'">'+e+'('+cbf[$_sys.getcbfindex(e,0)]+')</em>'
					 
				 })
			 }else{
				 jqbet+='<em data-val="'+jqs+'">'+jqs+'('+cbf[$_sys.getcbfindex(jqs,0)]+')</em>'
			 }
			 row.jqbet=jqbet;

    ...
    Class('Vs', {
        ...
        _init: function (){
            ...
            Y.get('#vsTable tr[isend=0]').hover(function (e, Y){//鼠标滑入滑出效果
    			...
    			this.children[13].style.backgroundColor = '#fffee6';
    			this.children[14].style.backgroundColor = '#fffee6';
    			...
    			this.children[13].style.backgroundColor = '';
...
//加载 期次-对阵
    Class('LoadExpect',{
    	...
    	LoadDuiZhen : function() {	
    		...
    		this.ajax({
    			        url: "/cpdata/match/jczq/"+uri+".json?v=" + Math.random(),
    			        cache:false,
    					end : function(data) {
    	...
    	jqs:function(data){
   		 ...
   		 var tableTpl=['',//0对阵头
              ...
              '<TBODY>'+
              '<TR style="DISPLAY: none" isend="1" odds="{$jqs}" dgsale="{$dsale}" lg="{$mname}" pdate="{$itemid}" pendtime="{$enddate}" pname="{$itemid}" class="{$classname}" mid="{$mid}" zid="{$itemid}">'+
              '<TD style="CURSOR: pointer"><LABEL for=m{$itemid} title="{$name}" {$dgstr}><INPUT id=m{$itemid} class=i-cr value={$itemid} CHECKED type=checkbox name=m{$itemid}>{$newname}</LABEL></TD>'+
              ...
              '</TR></TBODY>',//3隐藏对阵
              '<TBODY>'+
              '<TR isend="0" class="{$classname}" odds="{$jqs}" dgsale="{$dsale}" lg="{$mname}" pdate="{$itemid}" pendtime="{$enddate}" pname="{$itemid}" mid="{$mid}" zid="{$itemid}">'+
              '<TD style="CURSOR: pointer"><LABEL for=m{$itemid} title="{$name}" {$dgstr}><INPUT id=m{$itemid} class=i-cr value={$itemid} CHECKED type=checkbox name=m{$itemid}>{$newname}</LABEL></TD>'+
              '<TD style="BACKGROUND:{$cl}; COLOR: #fff" class=league><A title={$lmname} href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</A></TD>'+
              '<TD><SPAN class="eng end_time" title="开赛时间：{$mt}">{$short_et}</SPAN><SPAN style="DISPLAY: none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</SPAN> </TD>'+
              '<TD style="text-align: right; padding-right:2px;border-left:1px solid #ddd"><A title={$lhn} href="" target="_blank" id="hn{$itemid}"><em id="htid{$itemid}" class="ew_e ew_eright">{$hn}</em></A></TD>'+
              '<TD style="text-align: center;">VS</TD>'+
              '<TD style="text-align: left; padding-left:2px;"><A title={$lgn} href="" target="_blank" id="gn{$itemid}"><em id="gtid{$itemid}" class="ew_e">{$gn}</em></A></TD>'+
              '<TD style="CURSOR: pointer;border-left:1px solid #ddd" class=h_br><DIV class=label_br><INPUT class=chbox value=0 type=checkbox style="display: none"><SPAN class=eng>{$sp0}</SPAN></DIV></TD>'+
              ...
              '<TD style="CURSOR: pointer" class=h_br><DIV class=label_br><INPUT class=chbox value=4 type=checkbox style="display: none"><SPAN class=eng>{$sp4}</SPAN></DIV></TD>'+
              '<TD style="CURSOR: pointer" class=h_br><DIV class=label_br><INPUT class=chbox value=5 type=checkbox style="display: none"><SPAN class=eng>{$sp5}</SPAN></DIV></TD>'+
              '<TD style="CURSOR: pointer" class=h_br><DIV class=label_br><INPUT class=chbox value=6 type=checkbox style="display: none"><SPAN class=eng>{$sp6}</SPAN></DIV></TD>'+
              '<TD style="CURSOR: pointer" class=h_br><DIV class=label_br><INPUT class=chbox value=7 type=checkbox style="display: none"><SPAN class=eng>{$sp7}</SPAN></DIV></TD>'+
            ...
            this.get('#jjyh').click(function (e, Y){
            	var data = Y.C('choose_data'), minNum = Y.C('_isgg') ? 2 : 1,
                    ...
                    Y.checkLimitCode(data, chkgroup, function (){
                        ...
                        Y.get("#jjyh_form").attr("action", 'jjyh_jqs.html');

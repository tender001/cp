    ...
    Class('LoadExpect',{
    	...
    	spf:function(data){
    		 ...
    		 var tableTpl=['',//0对阵头
		           ...
		           '<td style="text-align: right; padding-right: 10px;border-left:1px solid #62A3D0;" class=h_br><div class="dz_dv" title="{$hn}"><s class="s_left"></s><span class="eng b span_left"><em class="em_left" id="htid_{$itemid}">{$hn}</em>{$closestr} <b class="b_left1">{$sp3}</b></span></div></td>'+
		           ...
		           '<td style="background:{$cl}; color: #fff" class=league><a title="{$lmname}" href="" target="_blank" id="mn{$itemid}" style="color: #fff">{$mname}</a></td>'+
		           '<td><span class="eng end_time" title="开赛时间：{$mt}">{$short_et}</span><span style="display: none" class="eng match_time" title="截止时间：{$et}">{$short_mt}</span></td>'+
		           '<td style="text-align: right; padding-right: 10px; cursor: pointer;border-left:1px solid #62A3D0" class=h_br><div class="dz_dv" title="{$hn}"><input class=chbox value=3 type=checkbox style="display: none"><s class="s_left">&nbsp;</s><span class="eng b span_left"><em class="em_left" id="htid_{$itemid}">{$hn}</em>{$closestr} <b class="b_left1">{$sp3}</span></div></td>'+

...
Class('CodeList>Z6CodeList', {
    noZero: true,
    lineTpl: '<span class="num" style="color:#333">{1} {2}</span> {3}<s></s><i></i>',
    createLine: function (code){//创建一行
        return this.createNode('LI', this.panel).html(this.lineTpl.format(code[0].join(','), this.getPlay()));
    },
    getType: function (){
    ...
    Class({
        ...
        createZhHz: function (){
            ...
            Y.lib.HzChoose({
                ...
                clear: '#zhxhz_clear',
                yl:[{
                    xml: '/cpdata/omi/53/yilou/hzyl_hz_all.xml',
                    //sort: 'hezhi',
                    offset: 0,
                    dom: '#zhxhz li.bqyl i'
                },{
        ...
        createTabs: function (){
           ...
           //主玩法
            ...
            playTabs.onchange = function (a, b){
            	...
            	this.get('#zxhz').html("和值");
            	if(b>0){this.get('#zxhz').html("组选和值");}
                ...
                this.get('#nav_normal label').slice(-5).hide(true).end().slice(-6, -5).hide(b>0)// 共用子玩法栏
            ...
            subPlayTabs.onchange = function (a, b){
                ...
                var tipId = ('#'+Class.config('play_name2')+pn[b]+'_tips').replace(/z\dhz/,'zxhz').replace('sc', 'pt');

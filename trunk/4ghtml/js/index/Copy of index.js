...
$_sys.grade_def.push([1, "一等奖,二等奖,三等奖,四等奖,五等奖,六等奖"]);
$_sys.grade_def.push([3, "直选,组选三,组选六"]);
$_sys.grade_def.push([4, "五星奖,三星奖,二星奖,一星奖,大小单双,二星组选,五星通选一等奖,五星通选二等奖,五星通选三等奖"]);
$_sys.grade_def.push([5, "和值,三同号通选,三同号单选,三不同号,三连号通选,二同号复选,二同号单选,二不同号"]);
$_sys.grade_def.push([6, "和值,三同号通选,三同号单选,三不同号,三连号通选,二同号复选,二同号单选,二不同号"]);
$_sys.grade_def.push([7, "一等奖,二等奖,三等奖,四等奖,五等奖,六等奖,七等奖"]);
$_sys.grade_def.push([8, "和值,三同号通选,三同号单选,三不同号,三连号通选,二同号复选,二同号单选,二不同号"]);
$_sys.grade_def.push([9, "和值,三同号通选,三同号单选,三不同号,三连号通选,二同号复选,二同号单选,二不同号"]);
...
$_sys_getwininfo = function(lotid, wininfo, pid) {
    var tmp = [];
    if (lotid == 85 || lotid == 86 || lotid == 87 || lotid == 88 || lotid == 89 || lotid == 84 || lotid == 90 || lotid == 91 || lotid == 92 || lotid == 93 || lotid == 94 || lotid == 95 || lotid == 96 || lotid == 97 || lotid == 70 || lotid == 71 || lotid == 72 || lotid == 99 || lotid == 98) {
...
function share() {
    $(".share").click(function() {
        if (localStorage.getItem("from") == "wxfw") {
            ...
            $("#ww_share").css({
                left: parseInt(document.documentElement.clientWidth / 2 - $("#ww_share").width() / 2),
                top: parseInt(document.documentElement.clientHeight / 2 - $("#ww_share").height())
            });
            $("#ww_share").show();
            $("#zhezhao").show()
        }
    ...
    $("#bf").bind("click",
    function() {
        if ($_lotid == 83 || $_lotid == 85 || $_lotid == 86 || $_lotid == 84 || $_lotid == 89) {
...
var WW = {
        ...
        $.ajax({
            ...
            success: function(xml) {
                ...
                if (code == "0") {
                    ...
                    var q = R.find("qcode");
                    var J = R.find("jindu");
                    var myjoins = R.find("myjoins");
                    var myjoin = myjoins.find("myjoin");
                    var myleng = myjoin.length;
                    var kjtime = J.attr("kjtime");
                    var gid = r.attr("gid");
                    var hid = r.attr("hid");
                    var itype = r.attr("type");
                    var icast = r.attr("cast");
                    var award = r.attr("award");
                    var pid = r.attr("pid");
                    var ireturn = r.attr("return");
                    var wininfo = r.attr("wininfo");
                    var owins = r.attr("owins");
                    var rmoney = r.attr("rmoney");
                    var cnickid = r.attr("cnickid");
                    var rnickid = r.attr("rnickid");
                    $_cnickid = rnickid;
                    var mulity = r.attr("mulity");
                    var lnum = parseInt(r.attr("lnum"));
                    var pnum = parseInt(r.attr("pnum"));
                    var aunum = r.attr("aunum");
                    var jindu = r.attr("jindu");
                    var views = r.attr("views");
                    var cdesc = r.attr("cdesc");
                    var ccodes = r.attr("ccodes");
                    var acode = q.attr("acode");
                    var etime = r.attr("etime");
                    var ifile = r.attr("ifile");
                    var tmoney = r.attr("tmoney");
                    var iopen = r.attr("iopen");
                    var wrate = r.attr("wrate");
                    var istate = r.attr("istate");
                    var rpmoney = r.attr("rpmoney");
                    var avg = r.attr("avg");
                    var tax = r.attr("tax");
                    var btime = r.attr("btime");
                    var source = r.attr("source");
                    if (itype == 1) {
                        $("#bf").hide();
                        ...
                        $("#rank").html("L" + aunum);
                        $("#rg").html(jindu + "%");
                        $("#bd").html("保&nbsp;" + Math.floor(pnum / tmoney * 100) + "%");
                        ...
                        $("#faje span").html(tmoney + "元&nbsp;(" + hb + mulity + "倍 )");
                        var kj = 0;
                        kj = WW.isKj(award, btime);
                        var isflg = 0;
                        if (istate > 0) {
                        ...
                        if (award == "2") {
                            ...
                            if (rmoney > 0) {
                                ...
                                if (lotid == 90 && r.attr("addmoney") > 0) {
                                    wininfostr += "发起人加奖金额：<font color=red>" + parseFloat(r.attr("addmoney")).rmb(true) + "</font>."
                                }
                                if (itype == 1 && istate != 3 && istate != 4) {
                                    wininfostr += "发起人提成：<font color=red>" + parseFloat(owins).rmb(true) + "</font>,每元派送<font color=red>" + parseFloat((tax - owins) / nums).rmb(true) + "</font>."
                                }
                            } else {
                                wininfostr += "未中奖"
                            }
            ...
            try {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var X = $(data).find("xml");
                        var row = X.find("row");
                        var mx = row.attr("code");
                        var matchs = row.attr("matchs");
                        $.ajax({
                                ...
                                r.each(function(aa) {
                                    ...
                                    if (c == 0) {
                                        ...
                                        if (hsstr.length > 0) {
                                            ...
                                            if (rt * 1 > 0) {
                                                bqcresult = bqcresult + "-3"
                                            } else if (rt * 1 == 0) {
                                                bqcresult = bqcresult + "-1"
                                            } else {
                                                bqcresult = bqcresult + "-0"
                                            }
                                        } else {
                                            bqcresult = ""
                                        }
                                            ...
                                            for (var i = 0; i < code.length; i++) {
                                                ...
                                                if (id == cc[0]) {
                                                    ...
                                                    for (var j = 0; j < touzhu.length; j++) {
                                                        ...
                                                        var bqc_i = ["1:0", "2:0", "2:1", "3:0", "3:1", "3:2", "4:0", "4:1", "4:2", "5:0", "5:1", "5:2", "9:0", "0:0", "1:1", "2:2", "3:3", "9:9", "0:1", "0:2", "1:2", "0:3", "1:3", "2:3", "0:4", "1:4", "2:4", "0:5", "1:5", "2:5", "0:9"];
                                                        var ii = bqc_i.indexOf(touzhu[j]);
                                                        if (bfresult != "") {
    ...
    lzc_clasli: function(lotid, expect, ccodes) {
        var gid = "";
        var exp = expect;
        switch (lotid + "") {
        case "80":

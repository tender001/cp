...
public class GamePlugin_82 extends GamePluginAdapter {

	public GamePlugin_82() {
		this.setGameID("82");
		this.setGradeNum(9);
		setGameName("进球彩");
	}
	...
	public GameAwardCode buildAwardCode(String awardCode) throws CodeFormatException {
		try {
			...
			String tmpcode = awardCode.trim().replace("*", "0123");
			...
			throw new CodeFormatException(1, "开奖号码不正确  正确格式为(0,1,2,3,*,1,2,3) 号码必须是[3210*] 必须是8个位置", awardCode);
		}
	}
	...
	public GameCastCode parseGameCastCode(String code) throws CodeFormatException {
		...
		if (cm == GameCastMethodDef.CASTTYPE_SINGLE || cm == GameCastMethodDef.CASTTYPE_MULTI) {// 单复式
			...
			for (int i = 0; i < len; i++) {
				...
				for (int j = 0; j < tlen; j++) {
					...
					if (intValue < 0 || intValue > 3) {
						throw new CodeFormatException(1, getGameName() + "号码只能为[3210]", code);
					}
					first |= 1L << (4 * i + intValue);
				}
			}
			...
			if (total < 8 || total > 32) {
				throw new CodeFormatException(1, getGameName() + " 号码个数必须在8-32之间", code);
			}
...
//				}
	...
	public static void main(String[] args) throws Exception {
		GamePlugin_82 plugin = new GamePlugin_82();
		GameAwardCode gac = plugin.buildAwardCode("2,3,3,0,0,1,3,1");

		long l = System.currentTimeMillis();

		// File file = new File("C:\\javawork\\caipiao\\GamePlugin\\82.txt");
		...
		// int[] ll = plugin.bingoMatch(gcc, gac, 9);
		// if (ll != null) {
		...
		String code = "013,013,013,013,013,013,013,013:1:2";
		GameCastCode[] gcc = plugin.parseGameCastCodes(code);

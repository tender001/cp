 ...
 *
 */
public class GamePlugin_98 extends GamePluginAdapter {

	private static final int MIN = 1;
	private static final int MAX = 32;
	
	public GamePlugin_98() {
		this.setGameID("98");
		this.setGameName("冠军");
		this.setGradeNum(32);
	}
	...
	public GameCastCode parseGameCastCode(String code) throws CodeFormatException {
		...
		if ( parts.length != 2 ) {
			throw new CodeFormatException(5, "格式不符合要求  必须是(GJ|号码)", code);
		}
		if ( !parts[0].equalsIgnoreCase("GJ")) {
			throw new CodeFormatException(5, "格式不符合要求  必须以GJ|开头", code);
		}
	...
	public static void main(String[] args) {
		GamePlugin_98 plugin = new GamePlugin_98();
		try {
			String codes = "GJ|14001=1/3/5/7/8/32";
			GameCastCode gcc = plugin.parseGameCastCode(codes);
			...
			HashMap<String,String> maps = plugin.keyBoardParser(codes, 1);

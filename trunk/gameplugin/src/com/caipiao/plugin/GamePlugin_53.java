...
public class GamePlugin_53 extends GamePluginAdapter {

	// 排列三玩法
	public static final byte PLS_SINGLE3 = 1; // 单选3
	public static final byte PLS_COMBINATION3 = 2; // 组选3
	public static final byte PLS_COMBINATION6 = 3; // 组选6
	public static final byte PLS_COMBINATION36 = 4; // 组合组选
	...
	private static int[] heshuCom36 = new int[] { 0, 2, 4, 4, 8, 10, 12, 16, 20, 22, 26, 28, 28, 30, 30, 28, 28, 26, 22, 20, 16, 12, 10, 8, 4, 4, 2, 0 };

	private static int[] baohaoSingle = new int[] { 0, 0, 12, 48, 120, 240, 420, 672, 1008, 1440 };
	...
	public GamePlugin_53() {
		this.setGameID("53");
		this.setGameName("排列三");
		this.setGradeNum(3);
	}
	...
	public int[] bingoMatcher(GameCastCode code, GameAwardCode bingoCode, int gradeNum) {
		...
		if (cm == GameCastMethodDef.CASTTYPE_BAOHAO) {// 包号
			...
			} else if (pm == PLS_COMBINATION3) {// 组3
				if (sames == 2) {
					int n = Long.bitCount(code.getFirst() & bingoCode.getFirst());
					n += Long.bitCount(code.getFirst() & bingoCode.getSecond());
					n += Long.bitCount(code.getFirst() & bingoCode.getThird());
			...
			} else if (pm == PLS_COMBINATION6) {// 组6
				if (sames == 0) {
					int n = Long.bitCount(code.getFirst() & bingoCode.getFirst());
					n += Long.bitCount(code.getFirst() & bingoCode.getSecond());
					n += Long.bitCount(code.getFirst() & bingoCode.getThird());
			...
			} else if (pm == PLS_COMBINATION36) {// 组3 6
				g = -1;
				if (sames == 2) {
					g = 1;
				} else if (sames == 0) {
	...
	public GameCastCode parseGameCastCode(String s) throws CodeFormatException {
		...
		try {
			...
			if (cm == GameCastMethodDef.CASTTYPE_BAOHAO) {// 包号
				...
				if (pm == PLS_SINGLE3) {// 直选
					money = baohaoSingle[f.length - 1];
				} else if (pm == PLS_COMBINATION3) {// 组三
					money = baohaoCom3[f.length - 1];
				} else if (pm == PLS_COMBINATION6) {// 组六
				...
				} else if (pm == PLS_COMBINATION36) {// 组三组六
					for (int i = 0; i < f.length; i++) {
						money += (heshuCom36[f[i]]);
					}
				} else {// 不支持的玩法
...
//		String strp = "";//密码
		...
		if ( gccs.length > 1) {
			...
			if ( cm == GameCastMethodDef.CASTTYPE_BAOHAO && gccs[0].getCastMoney() == 2 ) {
	...
	public static void main(String[] args) throws Exception {
		GamePlugin_53 p = new GamePlugin_53();

 ...
 *
 */
public class GamePlugin_84 extends GamePluginAdapter{
	
	private String gameID = BJUtil.SFGG;
	
	@Override
	public GameCastCode parseGameCastCode(String code) throws CodeFormatException {
		String[] parts = PluginUtil.splitter(code, "|");
		if ( ! parts[0].equalsIgnoreCase("SF")) {
			throw new CodeFormatException(5, "格式不符合要求  必须以SF|开头", code);
		}
		...
		if ( code.indexOf("$") > 0 ) {//胆拖
			...
			if (parts.length != 4 && parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是(SF|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (cparts.length != 2) {
				throw new CodeFormatException(12, "投注串不符合要求 胆拖必须是(SF|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是SF|号码|过关方式", code);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		GamePlugin_84 plugin = new GamePlugin_84();
		for (int i = 0; i < 1; i++) {
			GameCastCode gcc = plugin.parseGameCastCode("SF|1=3/0,2=3/0,3=3/0|3*1");
...
//		GamePlugin_85 plugin = new GamePlugin_85();

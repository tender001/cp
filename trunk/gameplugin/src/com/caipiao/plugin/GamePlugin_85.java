...
public class GamePlugin_85 extends GamePluginAdapter {
	
	private String gameID = BJUtil.SPF;

	@Override
	public GameCastCode parseGameCastCode(String code) throws CodeFormatException {
		String[] parts = PluginUtil.splitter(code, "|");
		if ( ! parts[0].equalsIgnoreCase("SPF")) {
			throw new CodeFormatException(5, "格式不符合要求  必须以SPF|开头", code);
		}
		...
		if ( code.indexOf("$") > 0 ) {//胆拖
			...
			if (parts.length != 4 && parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是(SPF|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (cparts.length != 2) {
				throw new CodeFormatException(12, "投注串不符合要求 胆拖必须是(SPF|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是SPF|号码|过关方式", code);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 1; i++) {
			GameCastCode gcc = plugin.parseGameCastCode("SPF|1=3/1/0,2=3/1/0,3=3/1/0|2*3");

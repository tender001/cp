...
public class GamePlugin_89 extends GamePluginAdapter {

	private String gameID = BJUtil.JQS;

	@Override
	public GameCastCode parseGameCastCode(String code) throws CodeFormatException {
		String[] parts = PluginUtil.splitter(code, "|");
		if ( ! parts[0].equalsIgnoreCase("JQS")) {
			throw new CodeFormatException(5, "格式不符合要求  必须以JQS|开头", code);
		}
		...
		if ( code.indexOf("$") > 0 ) {//胆拖
			...
			if (parts.length != 4 && parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是(JQS|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (cparts.length != 2) {
				throw new CodeFormatException(12, "投注串不符合要求 胆拖必须是(JQS|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是JQS|号码|过关方式", code);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		GamePlugin_89 plugin = new GamePlugin_89();
		for (int i = 0; i < 10000; i++) {
			GameCastCode gcc = plugin.parseGameCastCode("JQS|1=0,2=1,3=0,4=2,5=1|3*1");
			System.out.println(gcc.getCastMoney());
			
			gcc = plugin.parseGameCastCode("JQS|1=0,2=1$3=0,4=2,5=1,6=7,7=2,8=4|3*1|1");
			System.out.println(gcc.getCastMoney());
		}

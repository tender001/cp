...
function loginsh() {
	...
	$.ajax({
		url: 'login.aspx?fid=a_login',
		data: {uid:uid,pwd:pwd,yzm:yzm},
		type: 'POST',
		dataType: 'xml',
		error: function(){
			$("#uid").parent().parent().parent().prev('.alert').show().children('span').text('服务器连接失败！');
		},
		success: function(xml){
			...
			if(code == 0 ){
				...
				$("#uid").parent().parent().parent().prev('.alert').show().children('span').text(desc);
			}
		}
	});
}

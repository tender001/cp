...
public class GamePlugin_87 extends GamePluginAdapter {

	private String gameID = BJUtil.BQC;

	@Override
	public GameCastCode parseGameCastCode(String code) throws CodeFormatException {
		String[] parts = PluginUtil.splitter(code, "|");
		if ( ! parts[0].equalsIgnoreCase("BQC")) {
			throw new CodeFormatException(5, "格式不符合要求  必须以BQC|开头", code);
		}
		...
		if ( code.indexOf("$") > 0 ) {//胆拖
			...
			if (parts.length != 4 && parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是(BQC|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (cparts.length != 2) {
				throw new CodeFormatException(12, "投注串不符合要求 胆拖必须是(BQC|胆码$拖码|过关方式|胆容错数)", code);
			}
			...
			if (parts.length != 3) {
				throw new CodeFormatException(9, "格式不符合要求  必须是BQC|号码|过关方式", code);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		GamePlugin_87 plugin = new GamePlugin_87();
		for (int i = 0; i < 10000; i++) {
			GameCastCode gcc = plugin.parseGameCastCode("BQC|1=3-3,3=3-3,4=3-3/3-1/1-3|2*1");
			System.out.println(gcc.getCastMoney());
			
			gcc = plugin.parseGameCastCode("BQC|1=3:3,3=3:3$4=3:3/3:1/1:3|3*1");
			System.out.println(gcc.getCastMoney());
		}

 ...
 *
 */
public class GamePlugin_60 extends GamePluginAdapter {

	
//	private final static int[] Q2_BAOHAO = {0,
	
	private final static int[] nums = { 1, 2, 3, 4, 5, 6, 7, 8, 2, 3, 2, 3 };
	private final static int[] mins = { 1, 2, 3, 4, 5, 6, 7, 8, 2, 3, 2, 3 };
//	private final static int[] maxs = { 6, 3, 4, 7, 10, 8, 9, 8, 11, 11, 8, 9 };
	
	private final static int[] maxs = { 11, 11, 11, 11, 11, 11, 11, 8, 11, 11, 11, 11 };
	...
	public GamePlugin_60(){
		this.setGameID("60");
		this.setGameName("11选5");
		this.setGradeNum(12);
	}
...
//					int cf = len1 + len2 - Long.bitCount(first | second);
...
//				}
	...
	public HashMap<String, String> keyBoardParser(String codes,int muli) throws Exception {
		...
		if ( cm == GameCastMethodDef.CASTTYPE_SINGLE) {//单式
			...
			for (int i=0;i<gccs.length;i++) {
				String pcode = PluginUtil.longToString(gccs[i].getFirst());
				pcode += PluginUtil.longToString(gccs[i].getSecond());
				pcode += PluginUtil.longToString(gccs[i].getThird());
			...
			if ( pm == Q2) {
				str1 = toKeyString(PluginUtil.longToString(gccs[0].getFirst()));
				str2 = toKeyString(PluginUtil.longToString(gccs[0].getSecond()));
			} else if ( pm == Q3 ){
				str1 = toKeyString(PluginUtil.longToString(gccs[0].getFirst()));
				str2 = toKeyString(PluginUtil.longToString(gccs[0].getSecond()));
				str3 = toKeyString(PluginUtil.longToString(gccs[0].getThird()));
			} else {
				String pcode = PluginUtil.longToString(gccs[0].getFirst());
				str1 = toKeyString(pcode);
			}
	...
	public static void main(String[] args) throws Exception {
		GamePlugin_60 obj = new GamePlugin_60();
		String cc = "01,02,03,04,05,06,07$08,09:8:5";
//		cc = "02,03,04,06,09,11$01,08,10:08:05";// 选一前一直选
		cc = "02|02,03:9:2";
//		GameCastCode[] codes = obj.parseGameCastCodes(cc);
		
		HashMap<String,String> maps = obj.keyBoardParser(cc, 1);
